...
def list_loaded_models() -> list[str]:
    ...
    try:
        response = httpx.get(f"{LM_STUDIO_URL}/api/v0/models", headers=_headers(), timeout=5)
        ...
        try:
            response = httpx.get(f"{LM_STUDIO_URL}/v1/models", headers=_headers(), timeout=5)
            response.raise_for_status()
            return [
                model["id"]
                for model in response.json().get("data", [])
...
def is_available() -> bool:
    ...
    try:
        r = httpx.get(f"{LM_STUDIO_URL}/v1/models", headers=_headers(), timeout=5)
...
def chat(
    ...
    if not candidates:
        raise RuntimeError(
            "LM Studio inaccessible : aucun modele LLM charge. Configure LM_STUDIO_MODEL ou charge un modele dans LM Studio."
        )
    ...
    for candidate in candidates:
        ...
        try:
            ...
            if not can_retry:
                raise RuntimeError(
                    f"LM Studio erreur HTTP {exc.response.status_code}: {exc.response.text}"
                ) from exc
        except Exception as exc:
            raise RuntimeError(f"LM Studio inaccessible : {exc}") from exc
    ...
    if last_http_error is not None:
        raise RuntimeError(
            f"LM Studio erreur HTTP {last_http_error.response.status_code}: {last_http_error.response.text}"

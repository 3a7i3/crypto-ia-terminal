...
class RuntimeProvenanceInputs:
    ...
    # O-02W-C additive field (§15 equality invariant): the ONE
    # process_instance_id generated by the advisor bootstrap
    # (observability/process_identity.py). S-03 is a passive
    # consumer/projection of this value — it never generates or
    # reconciles its own. `exposure_epoch_id` below (S-03's pre-existing
    # process/exposure epoch) is unchanged in meaning and never aliased
    # to this field.
...
def build_snapshot(
    ...
    return {
        ...
        "process": {
            ...
            # O-02W-C additive field — propagated unchanged from the
            # advisor bootstrap's single identity authority
            # (observability/process_identity.py). Distinct from, and
            # never aliased to, exposure_epoch_id above (§15).

...
Endpoints:
    ...
    GET /api/rejections   → RejectionsSnapshot — breakdown/couche + rejets (JSON)
    GET /api/burnin       → BurnInSnapshot — progrès vs seuils statisticien (JSON)
    GET /api/regret       → RegretInvestigationSnapshot — MISSED_WIN/GOOD_REFUSAL (JSON)
...
@app.get(
    "/api/rejections",
    summary="RejectionStore breakdown + recent events (Reject Analyzer)",
)
...
@app.get(
    "/api/burnin", summary="Burn-in progress vs statistician thresholds (CLAUDE.md)"
)
def get_burnin():
...
@app.get(
    "/api/regret",
    summary="Regret breakdown by layer/regime/score/week (read-only investigation)",
)
def get_regret(regret_type: str = "MISSED_WIN"):
    return _regret_investigation_dict(
        load_regret_investigation(regret_type=regret_type)
    )


@app.get(
    "/api/decision/{packet_id}",
    summary="Full causal-chain trace for one decision packet",
)
def get_decision_packet(packet_id: str, days: int = 7):
    trace = load_decision_packet(packet_id, days=days)
    if trace is None:
        raise HTTPException(
            status_code=404, detail="Packet introuvable dans le RejectionStore"
        )

...
_log = get_logger("quant_hedge_ai.liquidity_map.flow_analyzer")
...
class SectorFlow:
    ...
    def opportunity_score(self) -> float:
        ...
        base = (
            min(50.0, self.whale_flow_usd / 1_000_000) if self.whale_flow_usd > 0 else 0
        )
...
class LiquidityFlowMap:
    ...
    def analyze(
            ...
            if whale_usd > 0 and not mapped:
                ...
                if re.search(r"\bSTABLECOIN\b", alert_upper):
                    ...
                    kw_path = os.path.join(
                        os.path.dirname(__file__), "sector_keywords.json"
                    )
                    with open(kw_path, "r", encoding="utf-8") as f:
                        kw_map = json.load(f)
                    # Liste de priorité configurable
                    priority_path = os.path.join(
                        os.path.dirname(__file__), "sector_priority.json"
                    )
        ...
        report = FlowReport(
            ...
            whale_mapping_coverage=(
                ((total_whale - unmapped_whale_total) / parsed_whale_total)
                if parsed_whale_total > 0
                else 1.0
            ),
    ...
    def render(self, report: FlowReport) -> str:
        ...
        for sf in report.sector_flows[:8]:
            icon = (
                "^"
                if sf.momentum_score > 0.1
                else ("v" if sf.momentum_score < -0.1 else "=")
            )
            lines.append(

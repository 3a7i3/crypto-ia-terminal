...
def test_flow_report_exposes_whale_consistency_metrics() -> None:
    ...
    report = fm.analyze(
        candles=candles, whale_alerts=whale_alerts, regime="test", cycle=1
    )
    ...
    found_stable = any(
        sf.sector == "STABLECOINS" and sf.whale_flow_usd == 2_400_000.0
        for sf in report.sector_flows
    )
    assert (
        found_stable
    ), "Unmapped whale should be mapped to STABLECOINS sector via keyword EXCHANGE"
...
def test_sector_inference_mapping() -> None:
    ...
    for sector, expected in mapping.items():
        total = sum(
            sf.whale_flow_usd for sf in report.sector_flows if sf.sector == sector
        )
        assert (
            total == expected
        ), f"Sector {sector} should have total whale flow {expected}, got {total}"

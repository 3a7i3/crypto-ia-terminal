...
def _make_candles(n: int = 100) -> list[dict]:
    import math

    base = 50_000.0
    result = []
    for i in range(n):
        price = base * (1 + 0.0005 * i + 0.02 * math.sin(i * 0.25))
        result.append(
            {
                "symbol": "BTCUSDT",
                ...
                "volume": round(1000 + i * 15, 2),
            }
        )
    return result


SAMPLE_CANDLES = _make_candles()
...
class TestEvolutionReport:
    ...
    def test_as_dict_best_strategy_name(self):
        r = EvolutionReport(
            best_strategy={"entry_indicator": "RSI", "exit_indicator": "MACD"}
        )
        d = r.as_dict()
...
class TestEvolutionEngine:
    ...
    def test_doctor_blocked_count(self, engine):
        ...
        assert (
            r.doctor_blocked + r.saved_to_memory <= r.backtests_run
            or r.doctor_blocked >= 0
        )

...
from quant_hedge_ai.market_radar.whale_tracker import WhaleTracker
...
def test_token_scanner_filter():
    scanner = TokenScanner(
        min_liquidity_usd=5_000, min_volume_usd=1_000, max_token_age_s=3600
    )
    all_tokens = [
        TokenInfo(
            symbol="GOOD",
...
def test_whale_tracker_scan():
    ...
    print(
        f"  [PASS] WhaleTracker.scan() → flow={report.smart_money_flow}, activities={len(report.activities)}"
    )
...
def test_social_scanner():
    ...
    print(
        f"  [PASS] SocialScanner.scan() → sentiment={report.overall_sentiment:.2f}, trending={report.trending_tokens}"
    )
...
def test_radar_full_sweep():
    ...
    print(
        f"  [PASS] MarketRadar.sweep() → {len(report.opportunities)} opportunities, risk={report.risk_level}"
    )

...
def test_apply_doctor_corrections_supports_custom_defaults() -> None:
    ...
    corrected = agent.apply_doctor_corrections(
        strategy, defaults={"trade_signal": "BUY", "allocation": 0.25}
    )
...
def test_run_realtime_demo_returns_corrected_snapshots_without_sleep() -> None:
    ...
    corrected = agent.run_realtime_demo(
        99, strategies, delay_seconds=0, send_func=_collector
    )
...
def test_director_panel_accumulates_entries() -> None:
    ...
    assert panel["performance_metrics"]["errors_corrected"] == len(
        agent.get_doctor_log()
    )

...
class StrategyScoreboard:
    ...
    def add(self, strategy: dict, metrics: dict) -> None:
        ...
        scoreboard.sort(
            key=lambda x: float(x["metrics"].get("sharpe", -999.0)), reverse=True
        )
        self.path.write_text(json.dumps(scoreboard[-500:], indent=2), encoding="utf-8")

    def get_active_strategies(self, n: int = 20, top_n: int = 0) -> list[dict]:
        return self.top(top_n if top_n > 0 else n)
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            "median_sharpe": (
                round(sorted(sharpes)[len(sharpes) // 2], 4) if sharpes else 0.0
            ),
        }

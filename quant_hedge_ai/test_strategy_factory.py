...
def test_validator_blocks_bad_strategy() -> None:
    ...
    decision = validator.validate(
        {"sharpe": 0.2, "drawdown": 0.25, "win_rate": 0.2, "pnl": -40}
    )
...
def test_validator_approves_good_strategy() -> None:
    validator = BotDoctorValidator()
    decision = validator.validate(
        {"sharpe": 2.8, "drawdown": 0.04, "win_rate": 0.62, "pnl": 25}
    )
...
def test_factory_run() -> None:
    ...
    report = factory.run(
        _sample_candles(), target_count=40, generations=1, regime="bull_trend"
    )
...
def test_factory_report_dict() -> None:
    factory = StrategyFactory()
    report = factory.run(
        _sample_candles(), target_count=30, generations=1, regime="sideways"
    )

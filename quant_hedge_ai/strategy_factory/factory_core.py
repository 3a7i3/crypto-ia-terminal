...
from quant_hedge_ai.strategy_factory.bot_doctor_validator import \
    BotDoctorValidator
from quant_hedge_ai.strategy_factory.performance_analyzer import \
    PerformanceAnalyzer
from quant_hedge_ai.strategy_factory.strategy_generator import \
    FactoryStrategyGenerator
...
class StrategyFactory:
    ...
    def run(
        ...
        candidates = self.generator.generate_candidates(
            population_size=target_count, generations=generations
        )
        ...
        memory_strategies = [
            r.get("strategy", {})
            for r in memory_loaded
            if isinstance(r.get("strategy"), dict)
        ]
        candidates.extend(memory_strategies)
        bt_data = history if history else candles
        backtested = self.backtester.run(candidates, bt_data)
        ...
        memory_saved = (
            self.memory.save_for_regime(regime, approved[:20]) if approved else 0
        )
        avg_loaded_age = (
            sum(float(item.get("age_cycles", 0)) for item in memory_loaded)
            / len(memory_loaded)
            if memory_loaded
            else 0.0
        )

...
from quant_hedge_ai.agents.market.regime_detector import RegimeDetector
...
from quant_hedge_ai.agents.research.feature_engineer import FeatureEngineer
...
from quant_hedge_ai.dashboard.ai_dashboard import render_console_report
...
def run_system(
    ...
    while True:
        ...
        paper_analyzer.analyze(
            [
                {
                    "title": "regime-adaptive trend",
                    "novelty": random.uniform(0.5, 1.0),
                    "relevance": random.uniform(0.4, 1.0),
                },
                {
                    "title": "cross-exchange mean reversion",
                    "novelty": random.uniform(0.5, 1.0),
                    "relevance": random.uniform(0.4, 1.0),
                },
            ]
        ...
        portfolio_weights = exposure_manager.cap(
            portfolio_weights, max_per_symbol=cfg.max_strategy_weight
        )
        ...
        report = render_console_report(
            heartbeat, best, performance, portfolio_weights, model_info
        )
...
def _build_runtime_from_args() -> RuntimeConfig:
    parser = argparse.ArgumentParser(
        description="Run V9 legacy autonomous quant system"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",

...
class AIControlCenter:
    ...
    def render_multi_sector_opps(self, flow_data: dict) -> str:
        ...
        if not opps:
            return "[MULTI] MULTI-SECTOR OPPORTUNITIES\n  None detected\n"
        lines = ["[MULTI] MULTI-SECTOR OPPORTUNITIES"]
        for opp in opps[:5]:
            lines.append(
    ...
    def render_header(self, cycle: int, timestamp: str) -> str:
        return (
            f"\n{'='*100}\n"
            f"[CONTROL] AI CONTROL CENTER - CYCLE {cycle} @ {timestamp}\n"
            f"{'='*100}\n"
        )
    ...
    def render_whale_radar(self, whale_data: dict) -> str:
        ...
        if sector_cov:
            sector_lines = [
                f"    {sector}: {cov:.2f}%" for sector, cov in sector_cov.items()
            ]
            sector_str = "\n".join(sector_lines)
        else:
    ...
    def render_strategy_performance(self, best: dict | None, stats: dict) -> str:
        if best is None:
            return "[BEST] BEST STRATEGY: None found\n"
    ...
    def render_portfolio(self, allocation: dict, brain_info: dict) -> str:
        ...
        return (
            f"[PORTFOLIO] PORTFOLIO ALLOCATION (Top 5)\n"
            f"    {alloc_str}\n"
    ...
    def render_decision(self, decision: dict) -> str:
        return (
            f"[EXECUTION] EXECUTION DECISION\n"

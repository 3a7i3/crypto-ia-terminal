"""Dashboard Director — central control center for the AI Hedge Fund system.
...
class DirectorDashboard:
    ...
    def update(
        ...
        *,
        ...
        # --- Agent Monitor ---
        if agent_errors > 0:
            self.agent_monitor.record_run(
                "system", error=True, output_summary=f"{agent_errors} errors"
            )
    ...
    def render(self, snapshot: DirectorSnapshot | None = None) -> str:
        ...
        if top_opps:
            top_str = "  ".join(
                f"{o['symbol']}({o['score']:.0f})" for o in top_opps[:3]
            )
        ...
        if fl:
            ...
            try:
                ...
                mapped_sectors = [
                    sd["sector"] for sd in sector_details if sd["whale_flow_usd"] > 0
                ]
                ...
                "findings": [
                    {
                        ...
                        "issue": "Spread within acceptable range",
                    },
                ],
            },

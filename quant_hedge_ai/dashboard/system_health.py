...
class SystemHealth:
    ...
    def evaluate(
        ...
        # Cycle speed
        avg_cycle_s = (
            sum(self._cycle_times) / len(self._cycle_times) if self._cycle_times else 0
        )
        ...
        components.append(
            ComponentHealth(
                ...
                status=(
                    "ok"
                    if agent_errors == 0
                    else ("warning" if agent_errors < 3 else "error")
                ),
                score=agent_score,
        ...
        doctor_status = (
            "ok"
            if doctor_score >= 80
            else ("warning" if doctor_score >= 50 else "error")
        )
        components.append(
            ComponentHealth(
                name="BotDoctor",
        ...
        components.append(
            ComponentHealth(
                ...
                status=(
                    "ok"
                    if risk_score >= 75
                    else ("warning" if risk_score >= 50 else "error")
                ),
                score=risk_score,
        ...
        components.append(
            ComponentHealth(
                ...
                status=(
                    "ok"
                    if pnl_pct >= -0.05
                    else ("warning" if pnl_pct >= -0.15 else "error")
                ),
                score=balance_score,
    ...
    def render(self, report: SystemHealthReport) -> str:
        ...
        icon = (
            ...
            else ("⚠️ " if report.status == "degraded" else "🚨")
        )
        ...
        for c in report.components:
            c_icon = (
                "✅" if c.status == "ok" else ("⚠️ " if c.status == "warning" else "❌")
            )

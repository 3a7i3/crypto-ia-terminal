...
_log = get_logger("quant_hedge_ai.dashboard.bot_doctor_panel")
...
class BotDoctorPanel:
    ...
    def record(
        ...
        _log.info(
            "BotDoctorPanel cycle %d: score=%.0f, status=%s", cycle, score, status
        )
        return report
    ...
    def render(self, report: DoctorPanelReport | None = None) -> str:
        ...
        icon = (
            "✅" if r.status == "healthy" else ("⚠️ " if r.status == "warning" else "🚨")
        )
        avg = self.avg_health()

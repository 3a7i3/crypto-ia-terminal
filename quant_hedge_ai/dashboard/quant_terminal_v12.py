...
if __name__ != "__main__" and (
    ...
    pytest.skip(
        "quant_terminal_v12.py is a Panel app and should not be tested with pytest.",
...
# ── allow imports from quant_hedge_ai root ───────────────────────────────
...
def candle_chart(coin: str = "BTC", tf: str = "1h") -> go.Figure:
    ...
    # Volume bars
    colors = [
        DARK["green"] if c >= o else DARK["red"] for c, o in zip(df.close, df.open)
    ]
    fig.add_trace(
        go.Bar(
            x=idx,
...
def portfolio_pie() -> go.Figure:
    ...
    fig = go.Figure(
        go.Pie(
            ...
            hole=0.45,
            marker=dict(
                colors=[DARK["green"], DARK["blue"], DARK["yellow"], DARK["red"]]
            ),
        ...
        rows.append(
            {
                ...
                "Amount $": amount,
                "Threat": (
                    "HIGH" if amount > 1e7 else ("MEDIUM" if amount > 2e6 else "LOW")
                ),
...
def risk_metrics() -> dict:
    ...
    sharpe = round(
        float(daily_rets.mean() / (daily_rets.std() + 1e-9) * np.sqrt(252)), 3
    )
...
def equity_curve() -> go.Figure:
    ...
    fig.add_hline(
        y=-10, line_dash="dash", line_color=DARK["yellow"], opacity=0.5, row=2, col=1
    )
    fig.add_hline(
        y=-20, line_dash="dash", line_color=DARK["red"], opacity=0.5, row=2, col=1
    )
...
def sharpe_bar(df: pd.DataFrame) -> go.Figure:
    ...
    colors = [
        DARK["green"] if s > 1.5 else DARK["yellow"] if s > 0.8 else DARK["red"]
        for s in top.Sharpe
    ]
    fig = go.Figure(
        go.Bar(
...
def agents_df() -> pd.DataFrame:
    ...
    for name, module in _AGENTS:
        status = random.choices(["running", "idle", "busy"], weights=[0.6, 0.25, 0.15])[
            0
        ]
...
run_lab_btn = pn.widgets.Button(
    name="⚡ Run Strategy Lab", button_type="success", width=180
)
...
# ─── KPI cards ───────────────────────────────────────────────────────────────
def _num(
    name: str, val: float, fmt: str = "{value:.2f}", color: str = "#ffffff"
...
market_table = pn.widgets.Tabulator(
    market_scan_df(50), **_table_cfg, widths=_MARKET_COL_WIDTHS
)
...
# ─── Risk panel ──────────────────────────────────────────────────────────────
_risk_md = pn.pane.Markdown(
    "",
    sizing_mode="stretch_width",
    styles={"background": DARK["panel"], "padding": "14px", "border-radius": "8px"},
)
...
def check_panel_status(module_name, file_hint=None):
    try:
        if file_hint:
            importlib.util.find_spec(file_hint)
        else:
            importlib.import_module(module_name)
        return "🟢 OK"
    except Exception:
        return "🔴 Erreur/module manquant"


def panel_button(label, command, status, url=None):
    btn = pn.widgets.Button(name=label, button_type="primary", width=260)
    if url:
        btn.js_on_click(args=dict(url=url), code="window.open(url, '_blank')")
    else:
        btn.on_click(command)
    return pn.Row(btn, pn.pane.Markdown(f"**{status}**", width=120))


def show_dashboard_quant_terminal(event=None):
    tabs.active = 0


def show_director_dashboard(event=None):
    tabs.active = 1  # à ajuster si onglet spécifique


def show_bot_doctor(event=None):
    tabs.active = 2  # à ajuster si onglet spécifique


def show_control_center(event=None):
    tabs.active = 3  # à ajuster si onglet spécifique


# Vérification des modules (simplifiée)
status_quant_terminal = check_panel_status("panel")
status_director = check_panel_status("quant_hedge_ai.dashboard.director_dashboard")
status_bot_doctor = check_panel_status("quant_hedge_ai.dashboard.bot_doctor_panel")
status_control_center = check_panel_status("quant_hedge_ai.dashboard.control_center")
status_evolution_3d = check_panel_status("streamlit")

home_panel = pn.Column(
    pn.pane.Markdown(
        """
# 🏠 Accueil — Navigation Dashboards
Sélectionnez un dashboard/panel à ouvrir. L'état de chaque module est affiché à droite.
""",
        width=600,
    ),
    panel_button(
        "Quant Terminal V12 (Panel)",
        show_dashboard_quant_terminal,
        status_quant_terminal,
    ),
    panel_button("Director Dashboard", show_director_dashboard, status_director),
    panel_button("Bot Doctor", show_bot_doctor, status_bot_doctor),
    panel_button("Control Center", show_control_center, status_control_center),
    panel_button(
        "Evolution 3D (Streamlit)",
        lambda e: None,
        status_evolution_3d,
...
template = pn.template.FastListTemplate(
    ...
    sidebar=[
        pn.pane.Markdown("### 🏠 Accueil rapide"),
        panel_button("Accueil", lambda e: tabs_with_home.__setattr__("active", 0), ""),
        pn.layout.Divider(),

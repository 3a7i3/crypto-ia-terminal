...
def print_report(all_passes: list[list[Measurement]]) -> None:
    ...
    print(
        "    MistakeMemory / RegretEngine : JSONL disk — lazy-load actif via ADVISOR_DEFER_OPTIONAL_INTEL=true"
    )
    print(f"    Total init   : {total_init:.0f}ms sequentiel")
    print(
        "    Warmup 1h    : ~3700ms reseau — demarre deja en paralele → couvrent le boot"
    )
    print(
        "    Gain reorder : deplacer scanners+warmup avant kill_switch → +~15-20ms de parallelisme"
    )
    print()
    print("=" * 88)
    print()

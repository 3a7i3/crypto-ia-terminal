...
class Position:
    ...
    # Time stop
    max_age_minutes: float = float(
        os.getenv("PM_MAX_AGE_MIN", "240")
    )  # 4h max par défaut
    time_stop_enabled: bool = True

    # Position aging — TP se resserre avec le temps si pas en profit
    aging_tp_decay: bool = bool(int(os.getenv("PM_AGING_DECAY", "1")))
    aging_start_min: float = float(
        os.getenv("PM_AGING_START", "60")
    )  # commence à dégrader après 1h

    # Liquidation defense — fermeture urgence avant liquidation
    liq_defense_pct: float = float(
        os.getenv("PM_LIQ_DEFENSE_PCT", "0.08")
    )  # ferme si dist < 8%
        ...
        if old_atr <= 0 or abs(atr - old_atr) / old_atr > 0.05:
            ...
            if self.side == PositionSide.LONG:
                ...
                if not self.break_even_done:
                    self.sl_price = (
                        max(self.sl_price, new_sl) if self.sl_price > 0 else new_sl
                    )
                self.tp_price = (
                    max(self.tp_price, new_tp) if self.tp_price_init > 0 else new_tp
                )
            else:
                new_sl = self.entry_price + sl_dist
                new_tp = self.entry_price - tp_dist
                if not self.break_even_done:
                    self.sl_price = (
                        min(self.sl_price, new_sl) if self.sl_price > 0 else new_sl
                    )
                self.tp_price = (
                    min(self.tp_price, new_tp) if self.tp_price_init > 0 else new_tp
                )
...
class PositionManager:
    ...
    def update_market_data(
        self, symbol: str, atr: float, volatility: float = 0.0
    ) -> None:
        """Propage les données de marché à toutes les positions ouvertes."""
        with self._lock:
    ...
    def _check_partial_close(self, pos: Position) -> None:
        ...
        if pos.pnl_pct >= pos.partial_close_trigger_pct:
            ...
            logger.info(
                ...
                pos.symbol,
                close_qty,
                pos.partial_close_pct * 100,
                pos.pnl_usd,
            )
            self._send_close_order(
                pos, qty_override=close_qty, reason=CloseReason.PARTIAL
            )
            pos.qty -= close_qty
    ...
    def _check_break_even(self, pos: Position) -> None:
        ...
        if pos.pnl_pct >= pos.break_even_trigger_pct:
            ...
            pos.sl_price = pos.entry_price * (
                1.0005 if pos.side == PositionSide.LONG else 0.9995
            )
            pos.break_even_done = True
    ...
    def _check_aging_tp(self, pos: Position) -> None:
        """
        Position aging — resserre le TP avec le temps si la position n'est pas en
        profit.

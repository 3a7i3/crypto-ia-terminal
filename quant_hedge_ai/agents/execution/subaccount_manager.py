...
_log = get_logger("quant_hedge_ai.agents.execution.subaccount_manager")
...
class SubaccountConfig:
    ...
    # Stratégie
    strategy: str = (
        "momentum"  # momentum | volatility | experimental | shadow | genetic
    )
...
class SubaccountUnit:
    ...
    def _init(self) -> None:
        ...
        _log.info(
            "[SubaccountUnit] %s initialisé (mode=%s, active=%s)",
            self.cfg.label,
            self.cfg.mode,
            self.cfg.active,
        )
    ...
    def _init_futures_exchange(self):
        try:
            ...
            _log.warning(
                "[SubaccountUnit] %s init futures échoué: %s", self.cfg.name, exc
            )
            return None
    ...
    def halt(self) -> None:
        self._halted = True
        _log.critical("[SubaccountUnit] %s HALTED", self.cfg.label)

    def resume(self) -> None:
        self._halted = False
        _log.info("[SubaccountUnit] %s repris", self.cfg.label)
    ...
    def create_order(self, symbol: str, action: str, size_usd: float) -> dict:
        ...
        try:
            ...
            reason = str(exc)
            _log.warning(
                "[SubaccountUnit] %s order rejected: %s", self.cfg.name, reason
            )
        ...
        if (
            result.get("mode") in ("futures_demo", "paper")
            and result.get("id")
            ...
            try:
                pos = Position(
                    ...
                    entry_price=float(
                        ...
                        or result.get("size_usd", size_usd)
                        / max(1, size_usd)
                        * size_usd
                        or 1
                    ),
                ...
                _log.warning("[SubaccountUnit] add_position échoué: %s", exc)
    ...
    def _place_futures(self, symbol: str, action: str, size_usd: float) -> dict:
        ...
        try:
            ...
            _log.info(
                "[SubaccountUnit] %s FUTURES %s %s qty=%.4f @ $%.2f id=%s",
                self.cfg.label,
                action,
                symbol,
                qty,
            ...
            _log.error("[SubaccountUnit] %s futures échoué: %s", self.cfg.name, exc)
            return {
                "symbol": symbol,
                "action": action,
                "size": size_usd,
    ...
    def stats(self) -> dict:
        return {
            ...
            "positions": (
                self._position_manager.stats() if self._position_manager else {}
            ),
...
class SubaccountManager:
    ...
    def __init__(self, configs: Optional[dict[str, SubaccountConfig]] = None) -> None:
        ...
        self._units: dict[str, SubaccountUnit] = {
            name: SubaccountUnit(cfg) for name, cfg in cfgs.items() if cfg.active
        }
        _log.info(
            "[SubaccountManager] %d subcomptes initialisés: %s",
            len(self._units),
    ...
    def halt_all(self) -> None:
        for u in self._units.values():
            u.halt()
        _log.critical("[SubaccountManager] TOUS les subcomptes HALTED")

    def resume_all(self) -> None:
        for u in self._units.values():
            u.resume()
        _log.info("[SubaccountManager] Tous les subcomptes repris")
    ...
    def route_signal(self, symbol: str, action: str, score: int) -> list[dict]:
        ...
        for unit in self.all_active():
            ...
            _log.info(
                "[SubaccountManager] Signal routé → %s: %s",
                unit.cfg.label,
                result.get("mode"),
            )
        return results
    ...
    def global_stats(self) -> dict:
        return {name: unit.stats() for name, unit in self._units.items()}

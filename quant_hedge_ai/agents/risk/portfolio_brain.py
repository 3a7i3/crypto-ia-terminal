...
class PortfolioBrain:
    ...
    # ── Limites globales ──────────────────────────────────────────────────────
    MAX_TOTAL_EXPOSURE_PCT  = float(os.getenv("PB_MAX_EXPOSURE_PCT",    "0.40"))  # 40% du capital max
    MAX_SINGLE_SYMBOL_PCT   = float(os.getenv("PB_MAX_SYMBOL_PCT",      "0.20"))  # 20% sur un seul actif
    MAX_SAME_REGIME_PCT     = float(os.getenv("PB_MAX_REGIME_PCT",      "0.30"))  # 30% dans même régime
    MAX_LEVERAGE_WEIGHTED   = float(os.getenv("PB_MAX_LEVERAGE",        "2.0"))   # levier agrégé max
    MAX_CORRELATION_RISK    = float(os.getenv("PB_MAX_CORRELATION",     "0.75"))  # risque corrélation max
    MIN_FRAGMENTATION_USD   = float(os.getenv("PB_MIN_POSITION_USD",    "30.0"))  # taille minimum
    MAX_POSITIONS           = int(os.getenv("PB_MAX_POSITIONS",         "5"))     # max positions simultanées
    MAX_SAME_DIRECTION      = int(os.getenv("PB_MAX_SAME_DIRECTION",    "3"))     # max long ou short
    ...
    def check_new_trade(
        ...
        if new_exposure_pct > self.MAX_TOTAL_EXPOSURE_PCT:
            ...
            if f <= 0:
                return PortfolioVerdict(
                    allowed=False, reason=f"Exposition totale trop élevée: {new_exposure_pct:.0%} > {self.MAX_TOTAL_EXPOSURE_PCT:.0%}",
                    size_factor=0.0, capital_available=0.0, warnings=warnings, metrics=snap.__dict__,
                )
        ...
        if sym_pct > self.MAX_SINGLE_SYMBOL_PCT:
            ...
            if f <= 0:
                return PortfolioVerdict(
                    allowed=False, reason=f"Concentration trop forte sur {symbol}: {sym_pct:.0%}",
                    size_factor=0.0, capital_available=0.0, warnings=warnings, metrics=snap.__dict__,
                )
        ...
        longs  = sum(1 for p in open_positions if getattr(p, "side", None) and p.side.value == "long")
        shorts = sum(1 for p in open_positions if getattr(p, "side", None) and p.side.value == "short")
        ...
        capital_avail = max(0.0, self._capital * self.MAX_TOTAL_EXPOSURE_PCT - snap.total_exposure_usd)

        if warnings:
            logger.info("[PortfolioBrain] Warnings: %s | factor=%.2f", " | ".join(warnings), final_factor)
    ...
    def rank_opportunities(
        self,
        candidates: list[dict],   # [{"symbol": ..., "score": ..., "regime": ..., "conviction": ...}]
        open_positions: list,
    ) -> list[dict]:
        """
        ...
        for c in candidates:
            ...
            ranked.append({**c, "adj_score": round(adj_score, 2), "sym_pct": round(sym_pct, 3)})
    ...
    def portfolio_health(self, open_positions: list) -> dict:
        ...
        return {
            ...
            "free_capital":        round(
                max(0, self._capital * self.MAX_TOTAL_EXPOSURE_PCT - snap.total_exposure_usd), 2
            ),
        }
    ...
    def _snapshot(self, positions: list) -> PortfolioSnapshot:
        ...
        snap.total_exposure_pct  = snap.total_exposure_usd / self._capital if self._capital else 0
    ...
    def _correlation_risk(self, new_symbol: str, positions: list) -> float:
        ...
        for p in positions:
            sym = getattr(p, "symbol", "?")
            key = (min(sym, new_symbol), max(sym, new_symbol))
            corr = self._corr_matrix.get((sym, new_symbol),
                   self._corr_matrix.get((new_symbol, sym), 0.5))
    ...
    def _portfolio_correlation_risk(self, positions: list) -> float:
        ...
        for a, b in pairs:
            ...
            corr  = self._corr_matrix.get((sym_a, sym_b),
                    self._corr_matrix.get((sym_b, sym_a), 0.5))
            risks.append(corr)
        return sum(risks) / len(risks)

    @staticmethod
    def _weighted_leverage(positions: list, new_size: float = 0, new_lev: int = 1) -> float:
        total_notional = sum(getattr(p, "size_usd", 0) * getattr(p, "leverage", 1)
                             for p in positions if not getattr(p, "closed", False))
        total_notional += new_size * new_lev
        total_size = sum(getattr(p, "size_usd", 0)
                         for p in positions if not getattr(p, "closed", False))
        total_size += new_size

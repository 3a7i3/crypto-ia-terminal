...
class CapitalAllocationEngine:
    ...
    EV_MIN_THRESHOLD    = float(os.getenv("CAE_EV_MIN",           "0.005"))  # EV min pour ouvrir un trade
    VOL_REFERENCE       = float(os.getenv("CAE_VOL_REFERENCE",    "0.015"))  # vol de référence (1.5%)
    VOL_MAX_REDUCTION   = float(os.getenv("CAE_VOL_MAX_REDUCTION","0.50"))   # réduction max à haute vol
    LEVERAGE_MAX        = float(os.getenv("CAE_LEVERAGE_MAX",     "3.0"))    # levier max absolu
    MIN_TRADES_KELLY    = int(os.getenv("CAE_MIN_TRADES_KELLY",   "10"))     # min trades pour Kelly
    ...
    def allocate(
        ...
        lev_vol_factor    = max(0.5, 1.0 - max(0.0, (volatility / self.VOL_REFERENCE - 1.0) * 0.3))
        adjusted_leverage = max(1.0, min(
            self.LEVERAGE_MAX,
        ...
        if size < base_size_usd * 0.7:
            reason = f"Taille réduite: kelly={kelly_fraction:.2f} ev={ev_score:.4f} vol={volatility:.3f}"

        logger.debug(
            "[CAE] size=%.2f lev=%.1f kelly=%.4f ev=%.5f vol=%.3f regime=%s conv=%.2f",
            size, adjusted_leverage, kelly_fraction, ev_score, volatility, regime, conviction_factor
        )
        ...
        try:
            ...
            if score:
                return {
                    ...
                    "avg_win_pct":      score.avg_pnl_pct * 2.0 if score.avg_pnl_pct > 0 else 0.03,
                    "avg_loss_pct":     abs(score.avg_pnl_pct) * 0.5 if score.avg_pnl_pct < 0 else 0.02,

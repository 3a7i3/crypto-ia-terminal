...
class ExecutiveOverride:
    ...
    def update(
        ...
        if daily_loss_pct is not None:
            m.daily_loss_pct = abs(daily_loss_pct) if daily_loss_pct < 0 else daily_loss_pct
    ...
    def check_trade(
        ...
        if level == OverrideLevel.VETO:
            reason = f"VETO EXECUTIF — {triggers[0] if triggers else 'protection capital'}"
    ...
    def _recalculate_level(self) -> None:
        ...
        if new_level != old_level:
            ...
            logger.warning(
                ...
                _LEVEL_NAMES[old_level], _LEVEL_NAMES[new_level],
            ...
            if self._on_change:
                try:
                    ...
                    logger.debug("[EO] callback erreur: %s", exc)

    def _compute_level(self) -> OverrideLevel:
        m = self._metrics

        # VETO — conditions absolues
        if (m.drawdown_pct >= self.DD_VETO
        ...
        # CAREFUL
        if (m.drawdown_pct  >= self.DD_CAREFUL

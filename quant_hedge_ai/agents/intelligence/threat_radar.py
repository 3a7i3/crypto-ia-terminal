...
FUNDING_EXTREME_THRESHOLD  = _env_float("RADAR_FUNDING_EXTREME",  0.003)   # 0.3% / 8h = extreme
...
CONSEC_CANDLES_SAME_DIR    = _env_float("RADAR_CONSEC_CANDLES",   7)       # 7 bougies mêmes direction = trend lock
RADAR_VETO_LEVEL           = os.getenv("RADAR_VETO_LEVEL", "CRITICAL")     # niveau qui bloque le trading
...
class ThreatRadar:
    ...
    def scan_sync(self, symbols: list[str] | None = None) -> RadarReport:
        ...
        report.trade_allowed = report.max_level not in (ThreatLevel.CRITICAL,) \
            if RADAR_VETO_LEVEL == "CRITICAL" else \
            report.max_level not in (ThreatLevel.HIGH, ThreatLevel.CRITICAL)
    ...
    def _check_volatility_spike(self, symbol: str, closes: list[float],
                                 highs: list[float], lows: list[float]) -> list[Threat]:
        threats: list[Threat] = []
        if len(closes) < 20:
            return threats
    ...
    def _check_price_momentum_lock(self, symbol: str, closes: list[float]) -> list[Threat]:
        threats: list[Threat] = []
        ...
        recent = closes[-(n+1):]
        directions = [1 if recent[i] > recent[i-1] else -1 for i in range(1, len(recent))]
    ...
    def _make_summary(self, report: RadarReport) -> str:
        ...
        parts = []
        for lvl in [ThreatLevel.CRITICAL, ThreatLevel.HIGH, ThreatLevel.MEDIUM, ThreatLevel.LOW]:
            if levels_count[lvl] > 0:
                parts.append(f"{lvl.value}:{levels_count[lvl]}")

        summary = f"RADAR [{'/'.join(parts)}] — " if parts else "RADAR — "
        top = sorted(report.threats, key=lambda t: [ThreatLevel.CRITICAL, ThreatLevel.HIGH,
                                                     ThreatLevel.MEDIUM, ThreatLevel.LOW,
                                                     ThreatLevel.NONE].index(t.level))
        for t in top[:2]:
    ...
    def format_telegram(self, report: RadarReport) -> str:
        ...
        for t in sorted(report.threats,
                        key=lambda x: [ThreatLevel.CRITICAL, ThreatLevel.HIGH,
                                       ThreatLevel.MEDIUM, ThreatLevel.LOW,
                                       ThreatLevel.NONE].index(x.level)):
            emoji = {"CRITICAL": "🔴", "HIGH": "🟠", "MEDIUM": "🟡", "LOW": "🔵"}.get(t.level.value, "⚪")

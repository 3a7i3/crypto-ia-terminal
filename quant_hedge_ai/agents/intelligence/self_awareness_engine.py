...
class SelfAwarenessEngine:
    ...
    Usage :
        ...
        engine.record_trade(pnl_pct, sharpe, regime, personality, latency_ms, slippage_pct)
    ...
    WR_DROP_CAUTION   = float(os.getenv("SA_WR_DROP_CAUTION", "0.15")) # winrate chute 15%
    WR_DROP_WARNING   = float(os.getenv("SA_WR_DROP_WARNING", "0.25")) # winrate chute 25%
    ...
    REVENGE_LOSS_SEQ  = int(os.getenv("SA_REVENGE_LOSSES",    "2"))    # pertes consec → risk revenge
    REVENGE_SIZE_MULT = float(os.getenv("SA_REVENGE_MULT",    "1.3"))  # taille augmente > seuil
    ...
    def record_trade(
        ...
        ts = time.time()
        self._trades.append({
            "ts": ts, "pnl": pnl_pct, "sharpe": sharpe,
            "regime": regime, "personality": personality,
        })
    ...
    def report(self) -> dict:
        ...
        return {
            ...
            "active_drifts":  [
                {"dim": d.dimension, "metric": d.metric, "msg": d.message,
    ...
    def _check_performance_drift(self) -> list[DriftSignal]:
        ...
        if wr_drop >= self.WR_DROP_WARNING:
            signals.append(DriftSignal(
                ...
                wr_recent, wr_baseline, DangerLevel.WARNING,
                f"Win rate chute {wr_drop:.0%} ({wr_baseline:.0%}→{wr_recent:.0%})",
            ))
        elif wr_drop >= self.WR_DROP_CAUTION:
            signals.append(DriftSignal(
                "performance", "win_rate",
                wr_recent, wr_baseline, DangerLevel.CAUTION,
                f"Win rate en baisse {wr_drop:.0%} ({wr_baseline:.0%}→{wr_recent:.0%})",
            ))
        ...
        if sh_drop >= self.SHARPE_DROP_WARN and sh_baseline > 0.3:
            signals.append(DriftSignal(
                "performance", "sharpe",
                sh_recent, sh_baseline, DangerLevel.WARNING,
        ...
        if cumdd >= self.DD_ACCEL_WARN:
            lvl = DangerLevel.DANGER if cumdd >= self.DD_ACCEL_WARN * 2 else DangerLevel.WARNING
            signals.append(DriftSignal(
                "performance", "drawdown_acceleration",
                cumdd, 0.0, lvl,
    ...
    def _check_behavioral_drift(self) -> list[DriftSignal]:
        ...
        if len(window) > self.OVERTRADE_MAX:
            signals.append(DriftSignal(
                ...
                len(window), self.OVERTRADE_MAX, DangerLevel.WARNING,
                f"Overtrading: {len(window)} ordres en {self.OVERTRADE_WINDOW}s (max {self.OVERTRADE_MAX})",
            ))
            ...
            if all(t["pnl"] < 0 for t in last_n):
                ...
                if len(sizes) >= 2 and sizes[-1] > sizes[0] * self.REVENGE_SIZE_MULT:
                    signals.append(DriftSignal(
                        ...
                        sizes[-1], sizes[0], DangerLevel.DANGER,
                        f"Pattern revenge trading détecté — {self.REVENGE_LOSS_SEQ} pertes + taille augmentée",
                    ))
        ...
        if len(self._personality_history) >= 10:
            ...
            if outliers >= 2:
                signals.append(DriftSignal(
                    "behavioral", "personality_drift",
                    float(outliers), 1.0, DangerLevel.CAUTION,
                    f"Dérive personnalité: {outliers}/3 trades hors mode dominant ({dominant})",
                ))
    ...
    def _check_market_mismatch(self) -> list[DriftSignal]:
        ...
        if unique >= 4 and len(recent) >= 8:
            signals.append(DriftSignal(
                "market", "regime_instability",
                float(unique), 2.0, DangerLevel.CAUTION,
                f"Régime très instable: {unique} régimes différents en {len(recent)} cycles",
            ))

        # Corrélation signal/résultat qui chute — stratégie dans mauvais régime
        wins_by_regime: dict[str, list[float]] = {}
        for t in recent:
            wins_by_regime.setdefault(t["regime"], []).append(1.0 if t["pnl"] > 0 else 0.0)
        for rg, outcomes in wins_by_regime.items():
            if len(outcomes) >= 4:
                wr = sum(outcomes) / len(outcomes)
                if wr < 0.35:
                    signals.append(DriftSignal(
                        "market", "regime_mismatch",
                        wr, 0.5, DangerLevel.WARNING,
    ...
    def _check_infra_drift(self) -> list[DriftSignal]:
        ...
        if self._latencies:
            ...
            if avg_lat >= self.LATENCY_CRIT_MS:
                signals.append(DriftSignal(
                    "infra", "latency",
                    avg_lat, self.LATENCY_WARN_MS, DangerLevel.DANGER,
                    f"Latence critique: {avg_lat:.0f}ms (seuil: {self.LATENCY_CRIT_MS:.0f}ms)",
                ))
            elif avg_lat >= self.LATENCY_WARN_MS:
                signals.append(DriftSignal(
                    "infra", "latency",
                    avg_lat, self.LATENCY_WARN_MS, DangerLevel.CAUTION,
        ...
        if self._slippages:
            ...
            if avg_slip >= self.SLIPPAGE_CRIT:
                signals.append(DriftSignal(
                    "infra", "slippage",
                    avg_slip, self.SLIPPAGE_WARN, DangerLevel.WARNING,
                    f"Slippage critique: {avg_slip:.3%}",
                ))
            elif avg_slip >= self.SLIPPAGE_WARN:
                signals.append(DriftSignal(
                    "infra", "slippage",
                    avg_slip, self.SLIPPAGE_WARN, DangerLevel.CAUTION,
    ...
    def _apply_level(self, level: DangerLevel, drifts: list[DriftSignal]) -> None:
        ...
        if level == DangerLevel.OK:
            ...
            logger.error(
                ...
                " | ".join(d.message for d in drifts if d.severity >= DangerLevel.DANGER),
            )

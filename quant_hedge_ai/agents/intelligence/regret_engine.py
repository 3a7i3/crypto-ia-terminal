...
_MIN_MOVE_PCT = float(os.getenv("REGRET_MIN_MOVE",   "0.015"))  # mouvement min pour qualifier
...
class RegretEngine:
    ...
    def register_candidate(
        ...
        logger.debug("[RegretEngine] Candidat enregistre: %s %s score=%d | refus: %s",
                     symbol, signal, score, refused_by)
        return candidate
        ...
        for c in self._candidates:
            ...
            if record:
                ...
                if record.regret_type == "MISSED_WIN":
                    ...
                    logger.info(
                        "[RegretEngine] REGRET: %s %s | signal prix=%.2f eval prix=%.2f "
                        "move=%.2f%% | refuse par: %s",
                        c.symbol, c.signal,
                        c.price_at_signal, price_now,
                        record.move_pct * 100,
                        c.refused_by,
                    )
    ...
    def stats(self) -> dict:
        ...
        avg_regret = (sum(r.regret_value for r in self._records) / len(self._records)
    ...
    def recent_regrets(self, n: int = 5) -> list[str]:
        missed = sorted(
            ...
            key=lambda x: x.ts_evaluated, reverse=True,
        )[:n]
        return [

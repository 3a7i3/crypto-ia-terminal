...
_log = get_logger("quant_hedge_ai.agents.intelligence.decision_quality_engine")
...
class DecisionQualityEngine:
    ...
    def evaluate_decision(
        ...
        if sig_score < 70:
            discipline_score -= (
                70 - sig_score
            ) * 1.5  # pénalise les trades sous le seuil
    ...
    def stats(self) -> dict:
        ...
        good_decisions = [
            r for r in closed if r.decision_quality >= self.QUALITY_THRESHOLD
        ]
    ...
    def _append_to_file(self, rec: DecisionRecord) -> None:
        try:
            ...
            _log.warning("[DQE] Erreur écriture: %s", exc)
        ...
        try:
            ...
            for line in lines[-500:]:
                ...
                if rec.classification == DecisionClass.PENDING:
                    ...
                    self._closed.append(rec)
            _log.info(
                "[DQE] Chargé: %d décisions (%d pending)",
                len(self._closed),
                len(self._pending),
            )
        except Exception as exc:
            _log.warning("[DQE] Erreur chargement: %s", exc)

...
Format des erreurs mémorisées (JSONL) :
  {
    ...
    "explanation": "Signal BUY dans un régime sideways — la stratégie momentum ne fonctionne pas en range",
    "context": { ... features ... },
...
class BlockRule:
    ...
    def matches(self, context: dict) -> bool:
        ...
        for key, val in self.conditions.items():
            if key == "max_score":
                ...
                if context.get("conviction_level") not in (val if isinstance(val, list) else [val]):
                    return False
            elif key == "max_vol":
    ...
    def describe(self) -> str:
        ...
        return f"[{self.error_type}] {cond_str} -> BLOCK | triggers={self.trigger_count}"
...
class MistakeMemory:
    ...
    Usage dans advisor_loop :
        ...
        # Avant un trade
        check = mm.check_before_trade(symbol, signal, score, regime, features, consecutive_losses)
        if check.blocked:
    ...
    MIN_LOSS_TO_ANALYZE    = float(os.getenv("MM_MIN_LOSS_PCT",    "-0.01"))   # -1% min pour analyser
    REPEAT_BLOCK_THRESHOLD = int(os.getenv("MM_REPEAT_THRESHOLD",  "2"))       # 2 erreurs similaires → règle
    RULE_EXPIRY_DAYS       = int(os.getenv("MM_RULE_EXPIRY_DAYS",  "30"))      # règles expirent après 30j
        ...
        for rule in self._rules:
            if rule.matches(context):
                ...
                logger.info(
                    ...
                    rule.rule_id, rule.explanation, rule.trigger_count, similar
                )
    ...
    def record_trade_result(
        ...
        error_type, explanation = self._classify_error(
            pnl_pct, signal, score, regime, conviction_level,
            context_features, signal_age_sec, consecutive_losses
        )
        ...
        record = {
            ...
            "context":          {
                ...
                for k in ("rsi", "atr_ratio", "momentum", "volume_ratio",
                          "macd_hist", "ema_bullish", "bb_pct", "trend_strength")
            },
            "signal_age_sec":     signal_age_sec,
            "consecutive_losses": consecutive_losses,
            "rule_generated":     None,
        ...
        if error_type not in (ErrorType.GOOD_TRADE, ErrorType.UNLUCKY):
            ...
            if similar_count >= self.REPEAT_BLOCK_THRESHOLD:
                rule = self._generate_rule(error_type, regime, signal, score,
                                           conviction_level, context_features, explanation)
                if rule:
                    record["rule_generated"] = rule.rule_id
                    self._rules.append(rule)
                    logger.warning(
                        "[MistakeMemory] NOUVELLE RÈGLE générée: %s | %s",
                        rule.rule_id, rule.describe()
                    )
        ...
        if error_type not in (ErrorType.GOOD_TRADE, ErrorType.UNLUCKY):
            logger.info(
                ...
                error_type, explanation, pnl_pct * 100
            )
        return record
    ...
    def _classify_error(
        ...
        if pnl_pct > self.MIN_LOSS_TO_ANALYZE:
            return ErrorType.UNLUCKY, f"Petite perte ({pnl_pct:.2%}) — probablement malchance"
        ...
        if atr_ratio > 0.03 and score < 80:
            return ErrorType.HIGH_VOL_ENTRY, (
                f"Entrée en haute volatilité (ATR={atr_ratio:.2%}) avec score {score}/100. "
                f"En haute vol, exiger score ≥80 et conviction HIGH."
            )
        ...
        if consecutive_losses >= 3:
            return ErrorType.CONSECUTIVE_LOSS, (
                ...
                f"Le marché n'est pas favorable. Réduire taille et attendre retournement."
            )

        # ── OVERTRADING ───────────────────────────────────────────────────────
        recent_losses = sum(
            1 for m in self._mistakes[-10:]
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            "error_rate":   round(
                (total - by_type.get(ErrorType.GOOD_TRADE, 0)
                       - by_type.get(ErrorType.UNLUCKY, 0)) / max(1, total), 3
            ),
        }

    def explain_last_mistakes(self, n: int = 5) -> list[str]:
        """Retourne les N dernières erreurs en texte lisible."""
        errors = [m for m in self._mistakes if m.get("error_type") not in
                  (ErrorType.GOOD_TRADE, ErrorType.UNLUCKY)]
    ...
    def _load(self) -> list[dict]:
        ...
        try:
            ...
            logger.warning("[MistakeMemory] Chargement partiel: %s", exc)
            ...
            if count >= self.REPEAT_BLOCK_THRESHOLD:
                ...
                if not existing:
                    rule = self._generate_rule(
                        et, regime, signal, 75, "medium", {}, f"Règle reconstruite: {et}"
                    )
                    if rule:
                        self._rules.append(rule)
        if self._rules:
            logger.info("[MistakeMemory] %d règles reconstruites depuis l'historique", len(self._rules))

    def _count_similar(self, error_type: str, regime: str, signal: str) -> int:
        return sum(
            1 for m in self._mistakes

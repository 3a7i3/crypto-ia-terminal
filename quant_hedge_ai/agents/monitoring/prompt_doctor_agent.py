...
class CreatePromptAgent:
    ...
    def run_realtime_demo(
        ...
        for strategy in strategies:
            corrected = self.simulate_telegram_alerts(
                user_id, strategy, send_func=send_func
            )
    ...
    def build_v26_compatible_report(self, issues: list[str]) -> dict[str, Any]:
        ...
        findings_sorted = sorted(
            findings, key=lambda item: int(item["priority"]), reverse=True
        )
        avg_priority = sum(int(item["priority"]) for item in findings_sorted) / max(
            len(findings_sorted), 1
        )
    ...
    def _apply_doctor_corrections_impl(
        ...
        for key, value in strategy_output.items():
            invalid_strings = {"error", "invalid", "nan"}
            is_invalid_str = (
                isinstance(value, str) and value.strip().lower() in invalid_strings
            )
        ...
        if isinstance(trade_signal, str):
            ...
            if normalized_signal in {"BUY", "SELL", "HOLD"}:
                ...
                corrected_strategy["trade_signal"] = str(
                    resolved_defaults.get("trade_signal", "HOLD")
                ).upper()
                issue = (
        ...
        if parsed_allocation is None:
            fallback_allocation = self._coerce_float(
                resolved_defaults.get("allocation")
            )
    ...
    def apply_doctor_corrections(
        ...
        corrected_strategy, _ = self._apply_doctor_corrections_impl(
            strategy_output, defaults=defaults
        )
    ...
    def build_evolution_snapshot(self, cycle_issues: list[str]) -> dict[str, Any]:
        ...
        risk_related = sum(
            1
            for item in self.doctor_log
            if "risk_level" in item or "allocation" in item
        )
    ...
    def export_v26_report(
        self, report: dict[str, Any], output_dir: str, cycle: int
    ) -> Path:
        output_path = Path(output_dir)
        output_path.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        out_file = output_path / f"bot_doctor_v26_cycle_{cycle}_{timestamp}.json"
        payload = {
            "cycle": cycle,
    ...
    def developer_dashboard(self) -> dict[str, Any]:
        return {
            ...
            "recent_corrections": self.doctor_log[-5:],
        }
    ...
    def interactive_doctor_panel(self, filter_by: str = "") -> dict[str, Any]:
        ...
        return {
            "filter": filter_by,
            "filtered_logs_count": len(filtered),
            "logs": filtered,
        }

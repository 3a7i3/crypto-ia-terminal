...
class StrategyRanker:
    ...
    PROMOTE_THRESHOLD  = float(os.getenv("RANKER_PROMOTE_SCORE", "65"))  # score ≥ 65 → promu
    DEMOTE_THRESHOLD   = float(os.getenv("RANKER_DEMOTE_SCORE",  "30"))  # score ≤ 30 → rétrogradé
    BLACKLIST_DEMOTES  = int(os.getenv("RANKER_BLACKLIST_DEMOTES", "3")) # 3 rétrogradations → blacklist
    ...
    def record_trade(
        ...
        score = self._scores.setdefault(key, StrategyScore(name=strategy_name, regime=regime))
        ...
        logger.info(
            ...
            strategy_name, regime, score.trades, pnl_pct * 100, composite,
        )
    ...
    def record_monte_carlo(self, strategy_name: str, regime: str, survival_rate: float) -> None:
    ...
    def top_strategies(self, regime: str, n: int = 5) -> list[StrategyScore]:
        ...
        candidates = [
            s for k, s in self._scores.items()
            if s.regime == regime and not s.blacklisted and s.trades >= self.MIN_TRADES_RANK
        ]
    ...
    def best_sharpe(self, regime: str) -> float:
        """Retourne le meilleur Sharpe moyen pour un régime (utilisé par LiveSignalEngine)."""
    ...
    def size_factor(self, strategy_name: str, regime: str) -> float:
        """
        Retourne un facteur [0.2, 1.5] à multiplier à la taille de base.
        ...
        if c >= self.PROMOTE_THRESHOLD:
            return min(1.5, 1.0 + (c - self.PROMOTE_THRESHOLD) / 100)
        if c <= self.DEMOTE_THRESHOLD:
            return max(0.2, c / self.DEMOTE_THRESHOLD * 0.5)
        return 1.0
    ...
    def auto_demote(self) -> list[str]:
        ...
        for key, score in list(self._scores.items()):
            if score.blacklisted:
                continue
            if score.staleness_days() > self.STALE_DAYS and score.trades < self.MIN_TRADES_RANK:
                del self._scores[key]
                forgotten.append(score.name)
                logger.info("[StrategyRanker] Oublié (stale): %s/%s", score.name, score.regime)
        if forgotten:
            self._save()
        return forgotten
    ...
    def _auto_rank(self, score: StrategyScore) -> None:
        ...
        if c >= self.PROMOTE_THRESHOLD and score.promoted == score.demoted:
            ...
            logger.info("[StrategyRanker] PROMU: %s/%s (score=%.1f)", score.name, score.regime, c)
        elif c <= self.DEMOTE_THRESHOLD:
            score.demoted += 1
            self._log_event("demoted", score, c)
            logger.warning("[StrategyRanker] RETROGRADE: %s/%s (score=%.1f demotion #%d)",
                           score.name, score.regime, c, score.demoted)
            if score.demoted >= self.BLACKLIST_DEMOTES:
                score.blacklisted = True
                self._log_event("blacklisted", score, c)
                logger.error("[StrategyRanker] BLACKLIST: %s/%s", score.name, score.regime)

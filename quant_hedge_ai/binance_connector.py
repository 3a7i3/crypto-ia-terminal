...
def _detect_mode() -> str:
    ...
    has_futures_demo = bool(
        os.getenv("BINANCE_FUTURES_DEMO_KEY") and os.getenv("BINANCE_FUTURES_DEMO_SECRET")
    )
...
class _PaperWallet:
    ...
    def place_order(self, symbol: str, side: str, amount_usdt: float, price: float) -> dict:
        ...
        if side.lower() == "buy":
            ...
            if self._balances.get("USDT", 0) < cost:
                return {"status": "rejected", "reason": "insufficient_funds", "mode": "paper"}
            ...
            if self._balances.get(asset, 0) < qty:
                return {"status": "rejected", "reason": "insufficient_asset", "mode": "paper"}
            self._balances[asset]           = self._balances.get(asset, 0) - qty
...
class BinanceConnector:
    ...
    def __init__(self, mode: Optional[str] = None) -> None:
        ...
        self._guard  = SessionGuard(
            ...
            max_consecutive_losses = int(os.getenv("EXEC_MAX_CONSEC_LOSSES",   str(_MAX_CONSEC_LOSSES))),
            max_order_size_usd     = float(os.getenv("EXEC_MAX_ORDER_USD",     str(_MAX_ORDER_USD))),
        )

        if self._mode == "paper":
            initial = float(os.getenv("PAPER_INITIAL_CAPITAL", str(_PAPER_CAPITAL_DEFAULT)))
    ...
    def _init_exchange(self):
        ...
        try:
            ...
            logger.error("[BinanceConnector] Erreur init exchange (%s) — fallback paper", exc)
            self._mode  = "paper"
            self._paper = _PaperWallet()
            return None
    ...
    def test_connection(self) -> dict:
        ...
        try:
            if self._mode == "paper":
                ...
                return {"status": "ok", "mode": "paper", "balance_usdt": usdt, "latency_ms": latency}

            bal     = self._exchange.fetch_balance()  # type: ignore[union-attr]
            usdt    = float(bal.get("free", {}).get("USDT", 0.0))
            latency = int((time.time() - t0) * 1000)
            logger.info("[BinanceConnector] Connexion OK — mode=%s balance=%.2f USDT lat=%dms",
                        self._mode, usdt, latency)
            return {"status": "ok", "mode": self._mode, "balance_usdt": usdt, "latency_ms": latency}

        except Exception as exc:
            logger.error("[BinanceConnector] test_connection erreur : %s", exc)
            return {"status": "error", "mode": self._mode, "error": str(exc), "latency_ms": -1}
    ...
    def place_order(
        ...
        if self._dedup.is_duplicate(symbol, side, amount_usdt):
            return {"status": "rejected", "reason": "duplicate_order", "mode": self._mode}
        ...
        try:
            ...
            if price <= 0:
                return {"status": "error", "reason": "price_fetch_failed", "mode": self._mode}
            ...
            logger.info(
                ...
                side, symbol, amount_usdt, price, order.get("status"),
            )
            return order
    ...
    def cancel_order(self, symbol: str, order_id: str) -> dict:
        ...
        try:
            ...
            return {"status": "cancelled", "order_id": order_id, "mode": self._mode, "raw": result}
        except Exception as exc:
    ...
    def get_price(self, symbol: str) -> float:
        ...
        if self._exchange is not None:
            try:
                ...
                logger.warning("[BinanceConnector] get_price(%s) erreur : %s", symbol, exc)

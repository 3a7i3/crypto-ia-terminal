...
def load_runtime_config_from_env() -> RuntimeConfig:
    return RuntimeConfig(
        ...
        population_size=get_env_int(
            "V9_POPULATION", 300, min_value=1, max_value=10_000
        ),
        ...
        max_drawdown=get_env_float(
            "V9_MAX_DRAWDOWN", 0.25, min_value=0.01, max_value=1.0
        ),
        min_sharpe_for_trade=get_env_float(
            "V9_MIN_SHARPE", 2.0, min_value=0.0, max_value=100.0
        ),
        trade_max_drawdown=get_env_float(
            "V9_TRADE_MAX_DRAWDOWN", 0.10, min_value=0.001, max_value=1.0
        ),
        whale_block_threshold=get_env_int(
            "V9_WHALE_BLOCK_THRESHOLD", 2, min_value=0, max_value=100
        ),
        max_risk_per_trade=get_env_float(
            "V9_MAX_RISK_PER_TRADE", 0.02, min_value=0.001, max_value=1.0
        ),
        whale_threshold_usd=get_env_float(
            "V9_WHALE_THRESHOLD", 500_000.0, min_value=1_000.0
        ),
        max_strategy_weight=get_env_float(
            "V9_MAX_POSITION_WEIGHT", 0.3, min_value=0.01, max_value=1.0
        ),
        monte_carlo_paths=get_env_int(
            "V9_MONTECARLO_SIMULATIONS", 200, min_value=10, max_value=100_000
        ),
        monte_carlo_steps=get_env_int(
            "V9_MONTECARLO_STEPS", 120, min_value=10, max_value=10_000
        ),
        display_frequency=get_env_int(
            "V9_DISPLAY_FREQUENCY", 1, min_value=1, max_value=1_000
        ),
        ...
        doctor_report_export_enabled=get_env_bool(
            "V9_DOCTOR_REPORT_EXPORT_ENABLED", False
        ),
        doctor_report_export_dir=get_env_str(
            "V9_DOCTOR_REPORT_EXPORT_DIR", "databases/doctor_reports"
        ),

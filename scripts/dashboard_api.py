...
class AuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        if not DASHBOARD_PASSWORD: return JSONResponse({"detail": "dashboard authentication not configured"}, status_code=503)
        ...
        if request.url.path.startswith("/api/"): return JSONResponse({"detail": "Unauthorized"}, status_code=401)

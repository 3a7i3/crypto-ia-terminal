...
def _load_audit_module() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "sqlite_contamination_audit_module", audit_path
    )
...
def load_audit_rules() -> tuple[list[tuple[str, str, str]], set[str], Path]:
    ...
    if not isinstance(heuristics, list) or not isinstance(failing, set):
        raise CleanupError(
            "HEURISTICS/FAILING introuvables dans sqlite_contamination_audit.py"
        )
...
def load_candidates(review_csv: Path, expected_ids: int) -> CandidateSet:
    ...
    with review_csv.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        if not required_fields.issubset(fieldnames):
            raise CleanupError(
                f"CSV invalide: colonnes attendues {sorted(required_fields)}, "
                f"trouvees {sorted(fieldnames)}"
            )
    ...
    if len(all_ids) != expected_ids:
        raise CleanupError(
            f"Precondition echouee: {len(all_ids)} ids uniques trouves "
            f"(attendu: {expected_ids})"
        )

    missing_rules = set(RULE_SQL) - rules_found
    if missing_rules:
        raise CleanupError(
            "Precondition echouee: regles manquantes dans le CSV: "
    ...
    if overlap:
        ...
        raise CleanupError(
            "Precondition echouee: intersection "
            f"CRITICAL_I12/HIGH_NO_PRICE_OK non vide: {shown}{suffix}"
        )
...
def validate_db_state(
    ...
    if overlap:
        ...
        suffix = " ..." if len(overlap) > 10 else ""
        raise CleanupError(
            f"Intersection en base entre {RULE_CRITICAL}/{RULE_HIGH}: {shown}{suffix}"
        )
...
def backup_candidates(
    ...
    if in_db != expected_ids:
        raise CleanupError(
            f"Precondition DB echouee: {in_db} lignes candidates presentes "
            f"(attendu: {expected_ids})"
        )
    ...
    with sqlite3.connect(str(backup_path), timeout=30) as backup_conn:
        ...
        backup_conn.executemany(
            f"INSERT INTO trades ({quoted_cols}) "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--confirm", action="store_true", help="autorise le DELETE puis le COMMIT"
    )
    ...
    try:
        with sqlite3.connect(str(db_path), timeout=30, isolation_level=None) as conn:
            ...
            try:
                saved = backup_candidates(
                    conn, backup_path, candidates.ids, args.expected_ids
                )
                if saved != args.expected_ids:
                    raise CleanupError(
                        f"Backup incomplet: {saved} lignes sauvegardees "
                        f"(attendu: {args.expected_ids})"
                    )
            ...
            if not args.confirm:
                rollback_if_needed(conn)
                print(
                    "\nAucun DELETE execute. Relancer avec --confirm pour "
                    "appliquer la suppression."
                )
                return 0

            _ph = _placeholders(len(candidates.ids))
            cursor = conn.execute(
                f"DELETE FROM trades WHERE id IN ({_ph})",
                tuple(candidates.ids),
            )
            deleted_rows = int(cursor.rowcount)
            if deleted_rows != args.expected_ids:
                raise CleanupError(
                    f"DELETE incomplet: {deleted_rows} lignes supprimees "
                    f"(attendu: {args.expected_ids})"
                )

...
def _next_gate(level: int, cert: dict[str, Any]) -> str:
    ...
    if level == 2:
        return f"FEATURE_DIP=true sur VPS + N>={50} decisions + OCS >= 90 (actuel: {cert['ocs']:.1f})"
    if level == 3:
...
def format_text_report(report: dict[str, Any]) -> str:
    ...
    for chk in report["checks"]:
        ...
        lines.append(f"  {icon} {chk['check_id']:<16}  {chk['name']:<36} {chk['duration_ms']:>5.0f}ms{score_str}")
        if chk["status"] == "FAIL":
    ...
    if hist["last_run"]:
        lines.append(f"  Last run:     {hist['last_run'][:19]} UTC  (Level {hist['last_level']})")
    lines.append("")
    ...
    lines.append(f"  H1-H6 authorized:   {'YES' if lineage['hypothesis_testing_authorized'] else 'NO'}")
    lines.append(f"  ACE authorized:     {'YES' if lineage['calibration_authorized'] else 'NO'}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Observer Certification Report Generator")

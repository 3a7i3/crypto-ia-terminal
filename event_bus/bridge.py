...
_log = get_logger("event_bus.bridge")
...
class SupervisionBridge:
    ...
    def _on_crash(self, event: CrashEvent) -> None:
        _log.error("[CRASH] contexte=%s erreur=%s", event.context, event.error)
        self._notify(
            f"💥 CRASH DÉTECTÉ\n"
    ...
    def _on_api_key_error(self, event: ApiKeyErrorEvent) -> None:
        _log.error("[API_KEY] exchange=%s erreur=%s", event.exchange, event.error)
        self._notify(
            f"🔑 Erreur Clé API\n"
    ...
    def _on_startup(self, event: SystemStartupEvent) -> None:
        _log.info("[STARTUP] mode=%s exchanges=%s", event.mode, event.exchanges)
        self._notify(
            f"🚀 Système Démarré\n"
            f"Mode: {event.mode}\n"
    ...
    def _on_shutdown(self, event: SystemShutdownEvent) -> None:
        ...
        _log.info("[SHUTDOWN] raison=%s uptime=%.1fh", event.reason, uptime_h)
        self._notify(
            f"🛑 Système Arrêté\n"
            f"Raison: {event.reason}\n"
    ...
    def _on_ws_stale(self, event: WsStaleEvent) -> None:
        _log.warning("[WS/STALE] %s depuis %.0fs", event.symbol, event.stale_seconds)
    ...
    def _notify(self, message: str, key: str = "") -> None:
        ...
        try:
            ...
            _log.debug("SupervisionBridge notifier error: %s", exc)

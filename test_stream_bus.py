"""
Test complet du StreamBus — mode simulation locale (sans exchange réel).
Lance : python3 test_stream_bus.py

Simule :
  - Flux orderbook avec spread variable
  - Flux trades avec bruit aléatoire
  - Trade whale déclenchant une alerte
  - Lecture snapshot non-bloquante depuis un "cycle ML"
  - Saturation de queue (drop test)
  - Reconnexion après erreur simulée
"""
...
class MarketSimulator:
    """
    Génère des ticks réalistes pour N symboles.
    Prix suit une marche aléatoire + sin pour simuler tendance.
    """
    ...
    def __init__(self, bus: StreamBus, tick_rate: float = 0.01):
        self.bus = bus
        self.tick_rate = tick_rate  # secondes entre chaque tick
    ...
    async def run(
        ...
        """Fait tourner la simulation pendant `duration` secondes."""
        start = time.time()
        whale_sent = False
        error_sent = False
        ...
        while time.time() - start < duration:
            ...
            for symbol in self.bus.symbols:
                # Marche aléatoire + composante sin (tendance)
                ...
                await self.bus._enqueue(
                    Tick(
                        ...
                        data={
                            ...
                            "change": drift / base * 100,
                        },
                    )
                )

            # Injection d'un whale trade à t=inject_whale_at
            if not whale_sent and elapsed >= inject_whale_at:
                ...
                whale_sent = True
                logger.info(
                    f"[SIM] Whale trade injecté — BTC/USDT BUY ${12.0 * price:,.0f}"
                )

            # Injection d'une erreur simulée à t=inject_error_at
...
# ══════════════════════════════════════════════════════════════════════
#  SIMULATEUR DE CYCLE ML
# ══════════════════════════════════════════════════════════════════════


class FakeMLCycle:
    """Simule le cycle orchestrateur qui lit le snapshot toutes les N ms."""
    ...
    async def run(self, duration: float) -> None:
        ...
        while time.time() - start < duration:
            ...
            eth_price = snap.get_mid_price("ETH/USDT")
            ...
            # Simule un traitement ML long (200ms)
            await asyncio.sleep(self.cycle_interval)

        logger.info(
            f"[ML] {self.cycles_run} cycles — "
            f"BTC last={self.prices_seen[-1]:,.2f} | "
            f"imbalance last={self.imbalances_seen[-1]:.3f}"
...
async def test_snapshot_population():
    """T1 — Le snapshot est peuplé après quelques ticks."""
    ...
    await asyncio.gather(
        sim.run(duration=0.5, inject_whale_at=99, inject_error_at=99),
    )
    ...
    check(
        "Tick count > 0",
        bus.snapshot.tick_count > 0,
        f"{bus.snapshot.tick_count} ticks",
    )
    await bus.stop()


async def test_mid_price_and_spread():
    """T2 — get_mid_price et get_spread retournent des valeurs cohérentes."""
    ...
    check(
        "Mid price dans plage BTC",
        mid is not None and 58_000 < mid < 72_000,
        f"{mid:,.2f}" if mid else "None",
    )
    check(
        "Spread positif",
        spread is not None and spread > 0,
        f"{spread*10000:.2f} bps" if spread else "None",
    )
    check(
        "Spread < 10 bps",
        spread is not None and spread < 0.001,
        f"{spread*10000:.2f} bps" if spread else "None",
    )
    await bus.stop()
...
async def test_orderbook_imbalance():
    """T3 — L'imbalance orderbook est entre -1 et +1."""
    ...
    imb = bus.snapshot.get_orderbook_imbalance("BTC/USDT")
    check("Imbalance non-None", imb is not None, str(imb))
    check(
        "Imbalance entre -1 et 1",
        imb is not None and -1.0 <= imb <= 1.0,
...
async def test_whale_detection():
    """T4 — Un trade > seuil déclenche une whale alert."""
    print("\n── T4 : Détection whale ──")
    ...
    check(
        "Whale alert dans snapshot",
        len(bus.snapshot.whale_alerts) > 0,
        f"{len(bus.snapshot.whale_alerts)} alertes",
    )
    check("Callback on_whale appelé", len(whale_alerts_received) > 0)
    if whale_alerts_received:
        a = whale_alerts_received[0]
        check("Alert contient symbol", "symbol" in a and a["symbol"] == "BTC/USDT")
        check("Alert contient side=buy", a.get("side") == "buy")
        check(
            "Montant > seuil",
            a.get("amount_usd", 0) >= 500_000,
...
async def test_corrupt_tick_resilience():
    """T5 — Un tick corrompu (None values) ne fait pas planter le bus."""
    ...
    check(
        "tick_count toujours croissant",
        bus.snapshot.tick_count > before_tick_count,
        f"{bus.snapshot.tick_count} ticks",
    )
    await bus.stop()
...
async def test_queue_saturation():
    """T6 — Saturation de queue : le bus drop proprement sans bloquer."""
    print("\n── T6 : Saturation de queue ──")
    bus = StreamBus(
        symbols=["BTC/USDT"],
        queue_maxsize=10,  # très petite queue pour forcer les drops
    )
    asyncio.create_task(bus.start())

    # Flood brutal : 500 ticks instantanés
    for i in range(500):
        await bus._enqueue(
            Tick(
                symbol="BTC/USDT",
    ...
    check(
        "Drop count > 0",
        bus.snapshot.drop_count > 0,
        f"{bus.snapshot.drop_count} drops",
    )
    ...
    check(
        "tick_count > 0",
        bus.snapshot.tick_count > 0,
        f"{bus.snapshot.tick_count} ticks traités",
    )
    await bus.stop()


async def test_ml_cycle_non_blocking():
    """T7 — Le cycle ML lit le snapshot sans bloquer l'ingestion."""
    print("\n── T7 : Cycle ML non-bloquant ──")
    bus = StreamBus(symbols=["BTC/USDT", "ETH/USDT", "SOL/USDT"])
    ml = FakeMLCycle(bus, cycle_interval=0.2)  # cycle ML toutes les 200ms
    sim = MarketSimulator(bus, tick_rate=0.008)  # ~125 ticks/s

    asyncio.create_task(bus.start())

    t_start = time.time()
    await asyncio.gather(
        sim.run(duration=2.0, inject_whale_at=1.0, inject_error_at=99),
        ml.run(duration=2.0),
    )
    ...
    check(
        "Prix reçus par le ML",
        len(ml.prices_seen) > 0,
        f"{len(ml.prices_seen)} snapshots lus",
    )
    check("Imbalances reçues par le ML", len(ml.imbalances_seen) > 0)
    check(
        "Bus tick count élevé",
        bus.snapshot.tick_count > 50,
        f"{bus.snapshot.tick_count} ticks pendant que ML tournait",
    )
    check(
        "Drop rate < 5%",
        bus.snapshot.drop_count / max(bus.snapshot.tick_count, 1) < 0.05,
        f"{bus.snapshot.drop_count / max(bus.snapshot.tick_count, 1):.1%}",
    )
    await bus.stop()


async def test_multi_symbol():
    """T8 — Les 3 symboles sont bien indépendants dans le snapshot."""
    ...
    check(
        "BTC et ETH ont des prix différents",
        btc is not None and eth is not None and btc != eth,
        f"BTC={btc:,.0f} ETH={eth:,.0f}" if btc and eth else "None",
    )
    check(
        "SOL dans la bonne plage",
        sol is not None and 100 < sol < 250,
        f"SOL={sol:.2f}" if sol else "None",
    )
...
async def test_stats():
    """T9 — La méthode stats() retourne des données cohérentes."""
    print("\n── T9 : Stats du bus ──")
    bus = StreamBus(symbols=["BTC/USDT", "ETH/USDT", "SOL/USDT"])
    asyncio.create_task(bus.start())
    sim = MarketSimulator(bus, tick_rate=0.005)
    ...
    s = bus.stats()
    check("tick_count dans stats", s["tick_count"] > 0, str(s["tick_count"]))
    check("drop_rate entre 0 et 1", 0 <= s["drop_rate"] <= 1.0, f"{s['drop_rate']:.2%}")
    check(
        "last_update_age < 2s",
        s["last_update_age"] < 2.0,
        f"{s['last_update_age']:.3f}s",
    )
    check("symbols_with_ob == 3", s["symbols_with_ob"] == 3, str(s["symbols_with_ob"]))
    await bus.stop()

...
def _setup_testnet() -> dict[str, str]:
    ...
    _print(
        "  Créez vos clés sur https://testnet.binance.vision → Log In with GitHub",
        _YELL,
    )
...
def _setup_futures_demo() -> dict[str, str]:
    ...
    _print(
        "  Créez vos clés sur https://demo.binance.com → Mon Compte → Gestion des API",
        _YELL,
    )
...
def run_setup() -> None:
    ...
    print(f"\n{'═'*60}")
    print(
        f"{_BOLD}{_CYAN}   🚀  Binance Connector — Assistant de configuration{_RESET}"
    )
    print(f"{'═'*60}")

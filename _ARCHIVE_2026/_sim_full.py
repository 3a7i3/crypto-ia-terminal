...
TRADES = [
    dict(
        ...
        conf=0.80,
        atr=0.015,
        path=[65000, 65300, 65700, 66000, 66300],
        expected="win",
    ),
    dict(
        symbol="ETH/USDT",
...
for i, t in enumerate(TRADES):
    ...
    rec = close_position(
        pos, t["exit_p"], "TP" if t["expected"] == "win" else "SL", write_log=False
    )
    ...
    records.append(rec)
    print(
        f"  [{i+1}] {t['symbol']} {t['direction']} {t['expected'].upper()} "
...
for ex in exits_in_log:
    ...
    pos_sim = {
        ...
        "take_profit": ex["entry_price"]
        * (1.02 if ex["direction"] == "long" else 0.98),
    }
    reason, ep = eng.check_path(pos_sim, pp)
    print(
        f"  {ex['symbol']} {ex['direction']}: "
...
for regime in regimes:
    ...
    assert best.get("type") in (
        "tp_sl",
        "trailing",
        "hybrid",
    ), f"type inconnu pour {regime}: {best.get('type')}"
    print(
        f"  {regime}: best={best['type']} score={best['score']:.6f} "

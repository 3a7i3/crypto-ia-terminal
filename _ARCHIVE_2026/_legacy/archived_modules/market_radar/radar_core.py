...
_log = get_logger("quant_hedge_ai._legacy.archived_modules.market_radar.radar_core")
...
class RadarReport:
    ...
    def as_dict(self) -> dict:
        ...
        return {
            ...
            "whale_flow": (
                self.whale_report.smart_money_flow if self.whale_report else "n/a"
            ),
            "social_sentiment": (
                round(self.social_report.overall_sentiment, 2)
                if self.social_report
                else 0.0
            ),
            "risk_level": self.risk_level,
            "anomaly_count": (
                len(self.anomaly_report.anomalies) if self.anomaly_report else 0
            ),
        }
...
class MarketRadar:
    ...
    def sweep(
        ...
        anomaly_report = self.anomaly_detector.detect(
            candles, features, combined_whale_alerts
        )
        ...
        opportunities = [
            o for o in opportunities if o.score >= self.min_opportunity_score
        ]
    ...
    def _build_opportunities(
        ...
        for token in tokens:
            ...
            social_contrib = 0.0
            matching_signals = [
                s for s in social_report.signals if s.symbol == token.symbol
            ]
            if matching_signals:
                avg_sentiment = sum(s.sentiment for s in matching_signals) / len(
                    matching_signals
                )
                mentions = sum(s.mention_count for s in matching_signals)
                social_contrib = min(
                    30.0, (avg_sentiment + 1.0) * 5.0 + min(mentions / 50, 10.0)
                )
            ...
            composite = max(
                0.0,
                min(
                    100.0, token_contrib + social_contrib + whale_contrib - risk_penalty
                ),
            )

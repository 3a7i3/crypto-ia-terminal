...
_log = get_logger(
    "quant_hedge_ai._legacy.archived_modules.market_radar.anomaly_detector"
)
    ...
    def _check_volume(self, candles: list[dict]) -> list[MarketAnomaly]:
        ...
        for c in candles:
            ...
            if avg_vol > 0 and vol > avg_vol * self.volume_spike_factor:
                ratio = vol / avg_vol
                severity = (
                    "critical" if ratio > 10 else ("high" if ratio > 5 else "medium")
                )
                anomalies.append(
                    MarketAnomaly(
        ...
        for c in candles:
            ...
            if abs(move) > self.price_move_threshold:
                ...
                severity = (
                    "critical"
                    if abs(move) > 0.25
                    else ("high" if abs(move) > 0.15 else "medium")
                )
                anomalies.append(
                    MarketAnomaly(
                        symbol=c.get("symbol", "UNKNOWN"),

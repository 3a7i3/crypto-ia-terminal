...
_log = get_logger("quant_hedge_ai._legacy.archived_modules.market_radar.whale_tracker")
...
class WhaleTracker:
    ...
    def get_alerts(self, report: WhaleReport) -> list[str]:
        ...
        for a in report.activities:
            if a.amount_usd >= self.threshold_usd:
                tag = a.action.upper()
                alerts.append(
                    f"WHALE_{tag}: {a.amount_usd / 1_000_000:.1f}M USD on {a.symbol}"
                )
    ...
    def _build_report(self, activities: list[WhaleActivity]) -> WhaleReport:
        ...
        for sym, v in symbols_seen.items():
            ...
            whale_amt = (
                v["buy"] if sym in accum else v["sell"] if sym in distrib else 0.0
            )
        ...
        for sector in sector_totals:
            ...
            if sector_totals[sector] > 0:
                coverage = round(
                    100.0 * sector_whale[sector] / sector_totals[sector], 2
                )
        ...
        return WhaleReport(
            ...
            alert_count=sum(
                1 for a in activities if a.amount_usd >= self.threshold_usd
            ),

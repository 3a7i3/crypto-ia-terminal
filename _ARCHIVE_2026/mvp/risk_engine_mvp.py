...
_log = get_logger("mvp.risk_engine_mvp")
...
class RiskEngineMVP:
    ...
    def evaluate(
        ...
        """
        # ── 1. No-Trade filter ────────────────────────────────────────────────
        filter_ok, filter_reason = self._no_trade_filter(
            symbol, direction, signal_score, signal_confidence
        )
        if not filter_ok:
            return RiskDecision(
                False, 0, 0, f"NO_TRADE: {filter_reason}", filter_ok=False
            )
        ...
        if not corr_ok:
            return RiskDecision(
                False, 0, 0, f"CORRELATION: {corr_reason}", correlation_ok=False
            )
    ...
    def register_open(
        self, symbol: str, direction: str, size_usd: float, entry_price: float
    ) -> None:
    ...
    def register_close(self, symbol: str, pnl_usd: float, pnl_pct: float) -> None:
        ...
        self._save_state()
        _log.info(
            "[RiskMVP] Close %s PnL=%.2f$ (%.2f%%) | capital=%.2f$ dd=%.1%%",
            symbol,
            pnl_usd,
            pnl_pct * 100,
            self._current_capital,
            self._current_drawdown() * 100,
        )
    ...
    def _no_trade_filter(
        ...
        if len(self._open_positions) >= self.max_positions:
            return (
                False,
                f"{len(self._open_positions)} positions ouvertes >= {self.max_positions}",
            )
        ...
        if self._consecutive_losses >= 3:
            return (
                False,
                f"{self._consecutive_losses} pertes consécutives — pause forcée",
            )
    ...
    def _current_drawdown(self) -> float:
        if self._peak_capital <= 0:
            return 0.0
        return max(
            0.0, (self._peak_capital - self._current_capital) / self._peak_capital
        )
    ...
    def _correlation_check(self, symbol: str, direction: str) -> tuple[bool, str]:
        ...
        if same_side >= self.max_same_side:
            return (
                False,
                f"{same_side} positions {direction} ouvertes >= {self.max_same_side}",
            )
        ...
        if len(related_open) >= 2:
            return (
                False,
                f"Trop d'actifs corrélés en {direction}: {[p['symbol'] for p in related_open]}",
            )
    ...
    def _compute_size(
        ...
        if current_dd >= self.soft_dd:
            dd_adj = max(
                0.3, 1.0 - (current_dd - self.soft_dd) / (self.hard_dd - self.soft_dd)
            )
        else:
            dd_adj = 1.0
        ...
        _log.debug(
            "[RiskMVP] size=%.2f$ kelly=%.3f vol_adj=%.2f dd_adj=%.2f conf_adj=%.2f",
            size_usd,
            kelly,
            vol_adj,
            dd_adj,
    ...
    def _save_state(self) -> None:
        try:
            ...
            _log.debug("[RiskMVP] save error: %s", exc)
    ...
    def _load_state(self) -> None:
        try:
            if _STATE_PATH.exists():
                ...
                _log.info(
                    "[RiskMVP] État chargé: capital=%.2f$ dd=%.1%%",
                    self._current_capital,
                    self._current_drawdown() * 100,
                )
        except Exception as exc:
            _log.debug("[RiskMVP] load error: %s", exc)

...
_log = get_logger("mvp.signal_engine_mvp")
...
class MVPSignal:
    ...
    def actionable(self) -> bool:
        return (
            self.score >= 60 and self.confidence >= 0.45 and self.direction != "neutral"
        )
...
class SignalEngineMVP:
    ...
    def __init__(self) -> None:
        ...
        self._train_X: dict[str, deque] = {
            t.value: deque(maxlen=2000) for t in SignalType if t != SignalType.NONE
        }
        self._train_y: dict[str, deque] = {
            t.value: deque(maxlen=2000) for t in SignalType if t != SignalType.NONE
        }
    ...
    def evaluate(
        ...
        if active:
            ...
            _log.debug(
                "[SignalMVP] %s best=%s score=%.0f conf=%.0%",
                symbol,
    ...
    def _mean_revert_signal(
        self, symbol: str, features: dict, market_state
    ) -> MVPSignal:
        """
        Mean reversion.
    ...
    def _apply_market_filter(
        self, signals: list[MVPSignal], market_state
    ) -> list[MVPSignal]:
    ...
    def _build_features(
        self, candles_1h: list, candles_4h: list | None
    ) -> dict[str, float]:
        try:
            c = candles_1h
            ...
            if len(closes) >= 35:
                prev_atrs = [
                    self._atr(
                        highs[i - 14 : i], lows[i - 14 : i], closes[i - 14 : i], 14
                    )
            ...
            if vols:
                vwap = sum(
                    closes[-20:][i] * vols[-20:][i] for i in range(min(20, len(closes)))
                ) / max(sum(vols[-20:]), 1e-8)
            ...
            _log.debug("[SignalMVP] feature error: %s", exc)
            return {}
    ...
    def _ml_confidence(
        self, signal_type: str, features: dict, heuristic_score: float
    ) -> float:
        """Retourne la confiance ML si modèle dispo, sinon heuristique."""
        model = self._models.get(signal_type)
        if (
            model is None
            or len(self._train_X.get(signal_type, [])) < self.MIN_SAMPLES_ML
        ):
    ...
    def _fit_one(self, sig_type: str, X_list: list, y_list: list) -> None:
        try:
            ...
            _log.info(
                "[SignalMVP] LightGBM '%s' entraîné sur %d samples", sig_type, len(X)
            )
        except Exception as exc:
            _log.warning("[SignalMVP] fit error %s: %s", sig_type, exc)
    ...
    def _save_model(self, name: str, model: Any) -> None:
        try:
            ...
            _log.debug("[SignalMVP] save error: %s", exc)
        ...
        for sig_type in ("momentum", "mean_revert", "breakout"):
            ...
            if path.exists():
                try:
                    ...
                    _log.info("[SignalMVP] Modèle '%s' chargé", sig_type)
                except Exception:
                    pass

...
class TradeRecord:
    ...
    fee_usd: float = 0.0
    # Attribution
    attribution: str = "unknown"    # signal | regime | execution | market | lucky | unlucky
    lesson: str = ""
...
class PostTradeLearning:
    ...
    def record(
        ...
        rec = TradeRecord(
            ...
            entry_score=entry_score, entry_confidence=entry_confidence,
        ...
        if self._signal_engine and entry_features:
            self._signal_engine.record_outcome(signal_type, entry_features, direction, pnl_pct)

        # Persistence
        self._persist(rec)

        logger.info("[Learning] %s | %s | %s→ %.2f%% $%.2f | %s | %s",
                    symbol, signal_type, direction, pnl_pct*100, pnl_usd,
                    rec.attribution, rec.lesson[:60] if rec.lesson else "")
    ...
    def _attribute(self, rec: TradeRecord) -> tuple[str, str]:
        ...
        if profitable and not good_signal:
            return "lucky", f"Signal faible (score={rec.entry_score:.0f}) mais trade rentable"

        if not profitable:
            # Quelle composante est responsable ?
            slip_excess = rec.actual_slippage_bps - rec.expected_slippage_bps
            if slip_excess > 20:
                return "execution_error", f"Slippage excessif: {rec.actual_slippage_bps:.0f}bps > attendu {rec.expected_slippage_bps:.0f}bps"
            ...
            if regime_wr is not None and regime_wr > 0.55 and not profitable:
                return "regime_mismatch", f"Régime '{rec.regime}' normalement profitable (WR={regime_wr:.0%}) — revérifier classification"

            if good_signal and not profitable:
                return "unlucky", f"Bon signal (score={rec.entry_score:.0f}) mais marché adverse"

            return "signal_error", f"Score faible ({rec.entry_score:.0f}) en {rec.regime} — réviser seuils"
            ...
            with _LOG_PATH.open("r", encoding="utf-8") as f:
                for line in f:
                    try:
                        ...
                        self._regime_pnl[d.get("regime", "unknown")].append(d.get("pnl_pct", 0))
                        self._signal_pnl[d.get("signal_type", "unknown")].append(d.get("pnl_pct", 0))
                        loaded += 1
                    except Exception:
                        continue
            if loaded:
                logger.info("[Learning] %d trades chargés depuis l'historique", loaded)
        except Exception as exc:
            logger.debug("[Learning] load error: %s", exc)

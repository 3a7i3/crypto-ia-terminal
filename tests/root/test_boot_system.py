    ...
    def _check_decision_chain(self) -> None:
        ...
        try:
            ...
            signal = LiveSignalEngine().evaluate(
                "BTC/USDT", {"1h": candles}, features=feat
            )
            ...
            self._print_result(
                "  MetaStrategyEngine", True, f"personality={perso.name}"
            )
            ...
            self._print_result("  NoTradeIntelligence", True, f"allowed={bool(nt)}")

            # Layer 7 — SelfAwareness
            aw = SelfAwarenessEngine().evaluate()
            self._print_result("  SelfAwarenessEngine", True, f"level={aw.level.name}")

            # Layer 8 — MistakeMemory
            mm = MistakeMemory()
            mm_check = mm.check_before_trade(
                "BTC/USDT", "BUY", signal.score, "bull_trend", feat
            )
            self._print_result("  MistakeMemory", True, f"blocked={mm_check.blocked}")
            ...
            all_ok = (
                gate.allowed
                and bool(nt)
                and not mm_check.blocked
                and pv.allowed
                and alloc.size_usd > 0
                and ev.allowed
    ...
    def _check_exchange(self) -> None:
        ...
        try:
            ...
            if ok_futures:
                ...
                self.results.append(
                    CheckResult(
                        "exchange.balance", ok, f"Futures=${bal:.0f} Spot=${cap:.0f}"
                    )
                )
        except Exception as e:
            short = str(e)[:100]

...
def test_load_entries_filters_by_symbol_and_cycle(service: DecisionTraceService, tmp_path: Path):
    day = date(2026, 6, 30)
...
def test_load_entries_dedups_by_packet_id(service: DecisionTraceService, tmp_path: Path):
    day = date(2026, 6, 30)
    _write_day(tmp_path, day, [
...
def test_load_entries_skips_malformed_lines(service: DecisionTraceService, tmp_path: Path):
    day = date(2026, 6, 30)
...
def test_load_recent_entries_spans_multiple_days(service: DecisionTraceService, tmp_path: Path):
    end = date(2026, 6, 30)
...
def test_trace_and_find_by_packet_id_across_days(service: DecisionTraceService, tmp_path: Path):
    end = date(2026, 6, 30)
    _write_day(tmp_path, end - timedelta(days=3), [_entry(packet_id="deep-pid", symbol="SOL/USDT")])
...
def test_statistics_aggregates_by_layer_regime_personality(service: DecisionTraceService):
    entries = [
        _entry(packet_id="p1", all_blockers=["meta_strategy(score<66)"], regime="bull_trend", personality_name="momentum"),
        _entry(packet_id="p2", all_blockers=["meta_strategy(score<66)"], regime="sideways", personality_name="scalp"),
        _entry(packet_id="p3", all_blockers=["portfolio(size=0)"], regime="bull_trend", personality_name="momentum"),
    ]
...
def test_rejections_snapshot_combines_stats_and_recent(service: DecisionTraceService, tmp_path: Path):
    day = date(2026, 6, 30)
    _write_day(tmp_path, day, [
...
def test_timeline_sorts_most_recent_first(service: DecisionTraceService, tmp_path: Path):
    day = date(2026, 6, 30)
    _write_day(tmp_path, day, [
...
def test_timeline_respects_limit(service: DecisionTraceService, tmp_path: Path):
    day = date(2026, 6, 30)
    _write_day(tmp_path, day, [_entry(packet_id=f"p{i}", ts=float(i)) for i in range(10)])

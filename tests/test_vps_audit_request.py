...
class TestImmutableGitRequests(unittest.TestCase):
    ...
    def test_service_trigger_matrix_is_allowlisted_end_to_end(self):
        ...
        pack = (root / "scripts/claude-service-trigger-matrix.py").read_text(
            encoding="utf-8"
        )

        self.assertIn("service_trigger_matrix", audit_request.ALLOWED_ACTIONS)
        self.assertIn("          - service_trigger_matrix\n", workflow)
        self.assertIn("    service_trigger_matrix)\n", dispatcher)
        self.assertIn("TRIGGER_CATALOG = (\n", pack)

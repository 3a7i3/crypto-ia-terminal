...
def _create_db(path: Path) -> None:
    with sqlite3.connect(str(path)) as conn:
        ...
        conn.executemany(
            """
            INSERT INTO trades (id, ts, symbol, status, price, error, mode,
                                 action, size, notional, order_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            [
                (
                ...
                (
                    ...
                    "rejected",
                    None,
                    "Order size $500.00 exceeds limit $100.00",
                    "paper",
                    "BUY",
                    500.0,
                    None,
...
def test_cleanup_aborts_on_rule_drift(tmp_path: Path) -> None:
    ...
    with review_csv.open("a", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            [99, 1780047999.0, "BTC/USDT", "ok", "", "", "HIGH_NO_PRICE_OK", "HIGH"]
        )

...
class TelegramKillSwitch:
    ...
    def start(self) -> None:
        ...
        if not TELEGRAM_TOKEN or not TELEGRAM_CHAT:
            _log.warning("[KillSwitch] Token/ChatID manquants — kill switch désactivé")
            return
        self._running = True
        self._thread = threading.Thread(target=self._poll_loop, daemon=True)
        self._thread.start()
        _log.info(
            "[KillSwitch] Polling Telegram démarré (interval: %ds)", POLL_INTERVAL
        )
        self._send_async(
    ...
    def _poll_loop(self) -> None:
        while self._running:
            try:
                ...
                _log.debug("[KillSwitch] Erreur polling: %s", exc)
    ...
    def _handle_update(self, update: dict) -> None:
        ...
        _log.info("[KillSwitch] Commande recue: %s", text)
    ...
    def _cmd_stop_all(self) -> None:
        ...
        _log.critical("[KillSwitch] STOP_ALL déclenché — halt immédiat")
        self._send(
            "STOP_ALL EXECUTE\n"
    ...
    def _cmd_close_all(self) -> None:
        ...
        _log.critical("[KillSwitch] CLOSE_ALL déclenché")
        self._send(
            "CLOSE_ALL EXECUTE\n"
        ...
        if self._on_close_all:
            try:
                ...
                _log.error("[KillSwitch] Callback close_all error: %s", exc)
    ...
    def _cmd_safe_mode(self) -> None:
        ...
        _log.warning("[KillSwitch] SAFE_MODE activé — bascule observation")
        self._send(
            "SAFE MODE ACTIVE\n"
        ...
        if self._on_safe_mode:
            try:
                ...
                _log.error("[KillSwitch] Callback safe_mode error: %s", exc)
    ...
    def _cmd_resume(self) -> None:
        ...
        _log.info("[KillSwitch] RESUME — reprise normale")
        status = []
        if was_halted:
    ...
    def _send(self, text: str) -> None:
        ...
        try:
            ...
            _log.debug("[KillSwitch] Send error: %s", exc)

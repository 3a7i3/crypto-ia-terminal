...
def print_trace(trace: DecisionTrace) -> None:
    print(f"\n{_BOLD}{'-'*60}{_RESET}")
    print(
        f"{_BOLD}[C{trace.cycle}] {trace.symbol}  {trace.side}  "
        f"score={trace.score}  régime={trace.regime}{_RESET}"
    )
...
def print_summary_table(entries: list[dict[str, Any]]) -> None:
    ...
    print(f"\n{_BOLD}{'='*70}{_RESET}")
    print(
        f"{_BOLD}  DECISION TRACE — RÉSUMÉ  "
        f"({len(entries)} entrées, {len(groups)} uniques){_RESET}"
    )
    print(f"{'='*70}{_RESET}")

    print(
        f"\n  {'CYC':>4}  {'SYMBOLE':<12}  {'SIDE':<5}  {'SCR':>3}  "
        f"{'RÉGIME':<15}  {'PERSONNALITÉ':<18}  BLOQUEUR"
    )
    ...
    for (cyc, sym), e in sorted(groups.items()):
        ...
        print(
            f"  {cyc:>4}  {sym:<12}  {side:<5}  {score:>3}  {regime:<15}  "
            f"{pers:<18}  {verdict_icon} {first_b}"
        )
...
def main() -> None:
    ...
    p.add_argument(
        "--summary",
        action="store_true",
        help="Résumé tabulaire seulement (pas de trace détaillée)",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=20,
        help="Nombre max d'entrées détaillées (défaut: 20)",
    )
    args = p.parse_args()
    ...
    entries = _service.load_entries(
        day=day, file=jsonl_path, symbol=sym_filter, cycle=args.cycle
    )
    ...
    if len(entries) > args.limit:
        print(
            f"\n{_DIM}[{len(entries) - args.limit} entrées supplémentaires — "
            f"utilisez --limit N pour en voir plus]{_RESET}"
        )

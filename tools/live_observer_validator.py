...
from dip.core.types import LAYER_DISPLAY, LAYER_ORDER, DecisionStatus, TimeRange, now_us
from dip.modules.causal_tree import CausalTreeBuilder
from dip.modules.counterfactual import PipelineSimulator
from dip.modules.decision_graph import DecisionGraphEngine, GraphBuilder
from dip.modules.decision_heatmap import HeatmapBuilder, _LAYER_DISPLAY_NAMES
from dip.modules.decision_replay import ReplayBuilder
from dip.modules.decision_timeline import TimelineBuilder

try:
    from dip.modules.explainability import ExplainabilityScoreEngine
...
CERT_HISTORY_PATH = _ROOT / "databases" / "certifications" / "observer_cert_history.jsonl"
VERSION = "1.0"
...
def _result_skip(check_id: str, name: str, reason: str, t0: float) -> LiveCheckResult:
    return LiveCheckResult(
        check_id=check_id, name=name, passed=False, skipped=True,
        duration_ms=round((time.perf_counter() - t0) * 1000, 1),
        details=f"SKIP -- {reason}", score=0.0, skip_reason=reason,
    )


def _result_pass(check_id: str, name: str, details: str, t0: float,
                 score: float = 100.0, metrics: dict | None = None) -> LiveCheckResult:
    return LiveCheckResult(
        check_id=check_id, name=name, passed=True, skipped=False,
        duration_ms=round((time.perf_counter() - t0) * 1000, 1),
        details=details, score=score, metrics=metrics or {},
    )


def _result_fail(check_id: str, name: str, details: str, t0: float,
                 score: float = 0.0, metrics: dict | None = None,
                 error: str | None = None) -> LiveCheckResult:
    return LiveCheckResult(
        check_id=check_id, name=name, passed=False, skipped=False,
        duration_ms=round((time.perf_counter() - t0) * 1000, 1),
        details=details, score=score, metrics=metrics or {}, error=error,
    )
...
def _make_obs(*, trade_allowed: bool = True, symbol: str = "BTCUSDT",
              regime: str = "SIDEWAYS", score: float = 75.0,
              blocker: Optional[str] = None, ts: Optional[float] = None,
              packet_id: Optional[str] = None) -> Any:
    obs = MagicMock()
...
def check_iv_live_001(live_mode: bool = False) -> LiveCheckResult:
    ...
    if live_mode:
        try:
            ...
            if n == 0:
                return _result_skip(CID, NAME, "DIPStore vide -- activer FEATURE_DIP=true", t0)
            rows = store.get_decisions(limit=n)
            null_ids = sum(1 for r in rows if not r.get("packet_id"))
            if null_ids == 0:
                return _result_pass(CID, NAME, f"{n} decisions indexees, 0 packet_id null", t0,
                                    metrics={"n_decisions": n, "null_ids": 0})
            return _result_fail(CID, NAME, f"{null_ids}/{n} decisions avec packet_id null", t0,
                                metrics={"n_decisions": n, "null_ids": null_ids})
        except Exception as e:
            return _result_fail(CID, NAME, "Erreur acces DIPStore production", t0, error=str(e))
    ...
    try:
        ...
        if n_stored == N:
            return _result_pass(CID, NAME, f"{N} obs injectees, {n_stored} persistees -- couverture 100%", t0,
                                metrics={"n_injected": N, "n_stored": n_stored, "coverage_pct": 100.0})
        return _result_fail(CID, NAME, f"{n_stored}/{N} persistees -- couverture {coverage:.0f}%", t0,
                            score=coverage, metrics={"n_injected": N, "n_stored": n_stored, "coverage_pct": coverage})
    except Exception as e:
...
def check_iv_live_002(live_mode: bool = False) -> LiveCheckResult:
    ...
    try:
        ...
        if ok_count == len(blockers):
            return _result_pass(CID, NAME, f"{len(blockers)}/{len(blockers)} rejets avec root_cause_layer valide", t0,
                                score=100.0, metrics={"n_tested": len(blockers), "n_valid": ok_count})
        return _result_fail(CID, NAME, f"{ok_count}/{len(blockers)} root_cause valides -- manquants: {missing}", t0,
                            score=score, metrics={"n_tested": len(blockers), "n_valid": ok_count, "missing": missing})
    except Exception as e:
        return _result_fail(CID, NAME, "Exception durant construction CausalTree", t0, error=str(e))
...
def check_iv_live_003(live_mode: bool = False) -> LiveCheckResult:
    ...
    if live_mode:
        ...
        try:
            ...
            conn.close()
            if count == 0:
                return _result_skip(CID, NAME, "trade_log.sqlite vide -- aucun trade execute", t0)
            return _result_pass(CID, NAME,
                                f"trade_log.sqlite: {count} trades -- chaine Decision->Trade active", t0,
                                metrics={"n_trades": count})
        except Exception as e:
            return _result_fail(CID, NAME, "Erreur lecture trade_log.sqlite", t0, error=str(e))
    ...
    try:
        ...
        if ok_count == N:
            return _result_pass(CID, NAME, f"{N}/{N} obs -- lifecycle complet (graph+timeline+causal)", t0,
                                score=100.0, metrics={"n_tested": N, "n_complete": ok_count})
        return _result_fail(CID, NAME, f"{ok_count}/{N} lifecycles complets", t0,
                            score=score, metrics={"n_tested": N, "n_complete": ok_count})
    except Exception as e:
...
def check_iv_live_004(live_mode: bool = False) -> LiveCheckResult:
    ...
    if live_mode:
        try:
            ...
            n = store.count_decisions()
            if n < MIN_DECISIONS:
                return _result_skip(CID, NAME, f"N={n} < MIN={MIN_DECISIONS} decisions de production", t0)
            conn = store._conn
            ...
            """).fetchone()[0]
            if orphans == 0:
                return _result_pass(CID, NAME, f"0 orphelin sur {n} decisions", t0,
                                    metrics={"n_decisions": n, "n_orphans": 0})
            return _result_fail(CID, NAME, f"{orphans} orphelins dans dip_counterfactuals", t0,
    ...
    try:
        ...
        score = ok_count / N * 100
        if ok_count == N:
            return _result_pass(CID, NAME, f"{N}/{N} CausalTrees sans reference orpheline", t0,
                                score=100.0, metrics={"n_tested": N, "n_clean": ok_count})
        return _result_fail(CID, NAME, f"{ok_count}/{N} CausalTrees propres", t0,
                            score=score, metrics={"n_tested": N, "n_clean": ok_count})
    except Exception as e:
...
def check_iv_live_005(live_mode: bool = False) -> LiveCheckResult:
    ...
    if live_mode:
        try:
            ...
            rows = store.get_decisions(limit=n)
            timestamps = [r.get("created_at") for r in rows if r.get("created_at") is not None]
            if len(timestamps) < 2:
                return _result_skip(CID, NAME, "Moins de 2 timestamps disponibles", t0)
            duplicates = len(timestamps) - len(set(timestamps))
            inversions = sum(1 for i in range(1, len(timestamps)) if timestamps[i] < timestamps[i - 1])
            if duplicates == 0 and inversions == 0:
                return _result_pass(CID, NAME, f"{len(timestamps)} timestamps: 0 doublon, 0 inversion", t0,
                                    metrics={"n_checked": len(timestamps), "duplicates": 0, "inversions": 0})
            return _result_fail(CID, NAME, f"{duplicates} doublons, {inversions} inversions", t0,
    ...
    try:
        ...
        if inversions == 0 and duplicates == 0:
            return _result_pass(CID, NAME, f"{N} timestamps: 0 inversion, 0 doublon", t0,
                                metrics={"n_checked": N, "inversions": 0, "duplicates": 0})
        return _result_fail(CID, NAME, f"{inversions} inversions, {duplicates} doublons", t0,
                            metrics={"n_checked": N, "inversions": inversions, "duplicates": duplicates})
    except Exception as e:
...
def check_iv_live_006(live_mode: bool = False) -> LiveCheckResult:
    ...
    if live_mode:
        try:
            ...
            """).fetchone()[0]
            if duplicates == 0:
                return _result_pass(CID, NAME, f"0 packet_id duplique sur {n} decisions", t0,
                                    metrics={"n_decisions": n, "duplicates": 0})
            return _result_fail(CID, NAME, f"{duplicates} packet_id dupliques detectes", t0,
                                metrics={"n_decisions": n, "duplicates": duplicates})
        except Exception as e:
            return _result_fail(CID, NAME, "Erreur requete duplicates", t0, error=str(e))
    ...
    try:
        ...
        n_stored = store.count_decisions()
        if n_stored == N:
            return _result_pass(CID, NAME, f"{N} IDs uniques, 0 doublon apres re-injection", t0,
                                metrics={"n_injected": N, "n_stored": n_stored, "duplicates": 0})
        extra = n_stored - N
        return _result_fail(CID, NAME, f"{extra} doublon(s): {n_stored} entrees pour {N} IDs", t0,
...
def check_iv_live_007() -> LiveCheckResult:
    ...
    try:
        ...
        if per_obs < MEMORY_BUDGET_BYTES_PER_OBS:
            return _result_pass(
                CID, NAME,
                f"{N} obs: +{total_growth / 1024:.1f} KB total, {per_obs:.0f} B/obs (budget: {MEMORY_BUDGET_BYTES_PER_OBS} B/obs)",
                t0, score=100.0,
                metrics={"n_obs": N, "total_growth_kb": round(total_growth / 1024, 2), "per_obs_bytes": round(per_obs)},
            )
        score = max(0.0, 100.0 - (per_obs - MEMORY_BUDGET_BYTES_PER_OBS) / 100)
        return _result_fail(
            CID, NAME,
            f"Fuite memoire: {per_obs:.0f} B/obs > budget {MEMORY_BUDGET_BYTES_PER_OBS} B/obs",
            t0, score=score,
            metrics={"n_obs": N, "total_growth_kb": round(total_growth / 1024, 2), "per_obs_bytes": round(per_obs)},
        )
    except Exception as e:
...
def check_iv_live_008(live_mode: bool = False) -> LiveCheckResult:
    ...
    try:
        for i in range(N):
            approved = (i % 2 == 0)
            blocker = LAYER_ORDER[i % len(LAYER_ORDER)] if not approved else None
            obs = _make_obs(trade_allowed=approved, blocker=blocker, packet_id=f"ivlive008_{i:04d}")
        ...
        score = ok_count / N * 100
        if ok_count == N:
            return _result_pass(CID, NAME, f"{N}/{N} replays deterministes et identiques", t0,
                                score=100.0, metrics={"n_tested": N, "n_ok": ok_count})
        return _result_fail(CID, NAME, f"{ok_count}/{N} replays identiques -- {mismatches}", t0,
                            score=score, metrics={"n_tested": N, "n_ok": ok_count, "mismatches": mismatches})
    except Exception as e:
    ...
    try:
        for i in range(N):
            ...
            if pid_ok and status_ok and dir_ok and count_ok:
                ...
                if not status_ok:
                    issues.append(f"status_mismatch(graph={graph.status.value},tree={tree.result},tl={tl.final_status.value})")
                if not dir_ok:
                    issues.append("direction_mismatch")
                if not count_ok:
                    issues.append(f"count_mismatch(nodes={len(graph.nodes)},steps={len(tl.steps)})")
        ...
        if ok_count == N:
            return _result_pass(CID, NAME,
                                ...
                                f" (explainability={'available' if _EXPLAINABILITY_AVAILABLE else 'absent'})",
                                t0, score=100.0, metrics={"n_tested": N, "n_ok": ok_count,
                                                          "explainability_available": _EXPLAINABILITY_AVAILABLE})
        return _result_fail(CID, NAME, f"{ok_count}/{N} coherents -- {inconsistencies[:3]}", t0,
                            score=score, metrics={"n_tested": N, "n_ok": ok_count, "issues": inconsistencies})
    except Exception as e:
...
def check_iv_live_010(live_mode: bool = False) -> LiveCheckResult:
    ...
    test_cases: list[tuple[Optional[str], bool]] = [(layer, False) for layer in LAYER_ORDER]
    ...
    try:
        ...
        if ok_count == total:
            return _result_pass(CID, NAME, f"{total}/{total} root_cause coherents (12 blockers + 3 APPROVED)", t0,
                                score=100.0, metrics={"n_tested": total, "n_ok": ok_count})
        return _result_fail(CID, NAME, f"{ok_count}/{total} root_cause coherents -- {errors[:3]}", t0,
                            score=score, metrics={"n_tested": total, "n_ok": ok_count, "errors": errors})
    except Exception as e:
        return _result_fail(CID, NAME, "Exception Root Cause Integrity", t0, error=str(e))
...
def _compute_iii(iv_live_results: list[LiveCheckResult]) -> dict[str, float]:
    ...
    def _avg(*ids: str) -> float:
        ...
        return round(sum(r.score for r in matches) / len(matches), 1) if matches else 100.0
...
def certify(
    ...
    if not iv_all_pass:
        ...
        failed_c = [r.check_id for r in iv_live_results if not r.passed and not r.skipped and r.check_id in critical_ids]
...
def save_history(results: list[LiveCheckResult], cert: dict[str, Any]) -> None:
    ...
    if CERT_HISTORY_PATH.exists():
        lines = [ln for ln in CERT_HISTORY_PATH.read_text(encoding="utf-8").strip().splitlines() if ln]
        seq = len(lines) + 1

    entry = {
        "certification_id": f"OBS-CERT-{datetime.now(timezone.utc).strftime('%Y')}-{seq:04d}",
        ...
        "checks": [
            {"id": r.check_id, "status": r.status, "score": r.score, "duration_ms": r.duration_ms}
            for r in results
        ],
    }
...
def print_report(results: list[LiveCheckResult], cert: dict[str, Any], verbose: bool = False) -> None:
    print("=" * 72)
    ...
    for r in results:
        ...
        print(f"  {icon} {r.check_id:<16}  {r.name:<36} {r.duration_ms:>6.0f}ms{score_str}")
    ...
    print("-" * 72)
    print(f"  IV-LIVE: {cert['n_pass']} PASS  {cert['n_fail']} FAIL  {cert['n_skip']} SKIP  |  {total_ms:.0f}ms")
    print(f"  III:     {cert['iii']:.1f}/100   (seuil Level 2: >=95)")
    print(f"  OCS:     {cert['ocs']:.1f}/100   (seuil Level 3: >=90)")
    if verbose:
        sub = cert["sub_scores"]
        print(f"  Sub:     Coverage={sub['sub_coverage']:.0f}%  Replay={sub['sub_replay']:.0f}%  "
              f"RootCause={sub['sub_root_cause']:.0f}%  Memory={sub['sub_memory']:.0f}%  "
...
def print_history(history: list[dict]) -> None:
    ...
    for entry in history[-20:]:
        ...
        print(
            ...
            f"L{entry['level']}  III={entry['iii']:.1f}  OCS={entry.get('ocs', 0):.1f}  "
...
def main() -> int:
    parser = argparse.ArgumentParser(description="INV-IV-LIVE: Live Observer Validation Suite")
    parser.add_argument("--live", action="store_true", help="Use production DIPStore")
    parser.add_argument("--check", metavar="ID", help="Run single check (e.g. IV-LIVE-006)")
    parser.add_argument("--json", action="store_true", dest="json_output", help="JSON output")
    parser.add_argument("--history", action="store_true", help="Show certification history")
    parser.add_argument("--report", action="store_true", help="Show detailed sub-scores")
    parser.add_argument("--export", metavar="FILE", help="Export full certification to JSON file")
    parser.add_argument("-v", "--verbose", action="store_true", help="Verbose per-check details")
    ...
    if args.export:
        ...
        export_path.write_text(json.dumps(output_dict, indent=2, ensure_ascii=True), encoding="utf-8")

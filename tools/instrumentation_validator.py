...
from dip.core.types import LAYER_DISPLAY, LAYER_ORDER, LayerStatus, TimeRange, now_us
...
def check_iv002(verbose: bool = False) -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            ...
            duration_ms=(time.time() - start) * 1000,
            details=(
                f"12/12 couches -> BLOCKED correct" if not errors
    ...
    try:
        for i in range(N):
            ...
            if rc.causing_layer not in LAYER_ORDER:
                errors.append(f"{pid}: causing_layer='{rc.causing_layer}' hors LAYER_ORDER")
            ...
            if rc.causing_layer != layer:
                errors.append(
                    f"{pid}: root_cause={rc.causing_layer}, attendu={layer}"
                )
        ...
        return CheckResult(
            ...
            duration_ms=(time.time() - start) * 1000,
            details=(
                f"{N} rejets -> RootCause unique et valide" if not errors
    ...
    try:
        for i in range(N):
            ...
            for j in range(1, len(graph.nodes)):
                ...
                if not any(e.source_node_id == src and e.target_node_id == tgt for e in graph.edges):
            ...
            if trade_allowed and len(graph.nodes) != len(LAYER_ORDER):
                errors.append(
                    f"{pid}: approuve mais {len(graph.nodes)} noeuds (attendu {len(LAYER_ORDER)})"
                )
        ...
        return CheckResult(
            ...
            duration_ms=(time.time() - start) * 1000,
            details=(
                f"{N} graphes complets et valides" if not errors
                else f"{len(errors)} erreurs sur {N}"
            ),
            metrics={"tested": N, "errors": len(errors)},
            error="\n".join(errors[:5]) if errors else None,
...
def check_iv005(verbose: bool = False) -> CheckResult:
    ...
    try:
        for i in range(N):
            ...
            for j in range(1, len(timeline.steps)):
                ...
                if curr.enter_timestamp_us < prev.exit_timestamp_us:
                    errors.append(
                        f"{pid}: step[{j}].enter < step[{j-1}].exit"
                    )
        ...
        return CheckResult(
            ...
            duration_ms=(time.time() - start) * 1000,
            details=(
                f"{N} timelines coherentes" if not errors
                else f"{len(errors)} erreurs sur {N}"
            ),
            metrics={"tested": N, "errors": len(errors)},
            error="\n".join(errors[:5]) if errors else None,
    ...
    try:
        for i in range(N):
            ...
            if s1.final_status != expected:
                errors.append(f"{pid}: final_status={s1.final_status}, attendu={expected}")
        ...
        return CheckResult(
            ...
            duration_ms=(time.time() - start) * 1000,
            details=(
                f"{N} replays deterministes et fideles" if not errors
                else f"{len(errors)} erreurs sur {N}"
            ),
            metrics={"tested": N, "errors": len(errors)},
            error="\n".join(errors[:5]) if errors else None,
    ...
    try:
        ...
        for layer in LAYER_ORDER:
            ...
            for target in LAYER_ORDER[:4]:
                ...
                scenarios_run += 1
                if s1 != s2:
                    errors.append(f"blocker={layer},target={target}: status {s1} != {s2}")
                if abs(c1 - c2) > 1e-9:
        ...
        return CheckResult(
            ...
            duration_ms=(time.time() - start) * 1000,
            details=(
                f"{scenarios_run} simulations reproductibles" if not errors
...
def check_iv008(verbose: bool = False) -> CheckResult:
    ...
    try:
        ...
        if len(hm_sym.axes.x_values) != 12:
            errors.append(f"SymbolxLayer: {len(hm_sym.axes.x_values)} couches (attendu 12)")
        if hm_sym.total_decisions != len(rows):
            errors.append(f"SymbolxLayer: total_decisions={hm_sym.total_decisions} != {len(rows)}")
        ...
        if len(hm_reg.axes.x_values) != 12:
            errors.append(f"RegimexLayer: {len(hm_reg.axes.x_values)} couches (attendu 12)")
        ...
        return CheckResult(
            ...
            duration_ms=(time.time() - start) * 1000,
            details=(
                f"12/12 couches, {len(hm_sym.cells)} cellules (symbol), {len(hm_reg.cells)} (regime)"
                if not errors else f"{len(errors)} erreurs"
            ),
            metrics={
    ...
    try:
        for i in range(N):
            ...
            if rc_layer not in cn_layers:
                errors.append(f"{pid}: root_cause.causing_layer='{rc_layer}' absent de causal_nodes")

            # overall_confidence in [0,1]
            if not (0.0 <= tree.overall_confidence <= 1.0):
                errors.append(f"{pid}: overall_confidence={tree.overall_confidence:.3f} hors [0,1]")
        ...
        return CheckResult(
            ...
            duration_ms=(time.time() - start) * 1000,
            details=(
                f"{N} CausalTrees acycliques et valides" if not errors
                else f"{len(errors)} erreurs sur {N}"
            ),
            metrics={"tested": N, "errors": len(errors)},
            error="\n".join(errors[:5]) if errors else None,
...
def check_iv010(verbose: bool = False) -> CheckResult:
    ...
    try:
            ...
            if row and row.get("graph_json"):
                ...
                for k in range(1, len(ts_list)):
                    if ts_list[k] < ts_list[k - 1]:
                        errors.append(
                            f"Graph {pid[:8]}: node[{k}].ts={ts_list[k]} < node[{k-1}].ts={ts_list[k-1]}"
                        )
                        break
        ...
        return CheckResult(
            ...
            duration_ms=(time.time() - start) * 1000,
            details=(
                f"{N} decisions, timestamps monotones" if not errors
                else f"{len(errors)} erreurs sur {N}"
            ),
...
def generate_report(
    ...
    if cert["status"] == "CERTIFIED_OBSERVER":
        ...
        lines.append(f"  [!!] DIP CONDITIONNEL -- checks a corriger: {', '.join(failed_ids)}")
        ...
        lines.append("       Instrumentation defectueuse -> donnees non fiables -> S1 interdit.")

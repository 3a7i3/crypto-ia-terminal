...
| Global risk gate pass/fail | `quant_hedge_ai/agents/risk/global_risk_gate.py` (the current SOURCE decision-path gate — verified wired via `core/advisor_runtime_adapters.py`/`core/advisor_loop.py`; SOURCE PROOF != RUNTIME PROOF, actual VPS runtime use is RUNTIME_PROOF_REQUIRED; the older `risk/global_risk_gate.py` is LEGACY/superseded and not wired into any current source entrypoint, see `MODULE_FAMILY_REGISTRY.md`) | `HUMAN_DIAGNOSTIC` | `QUANT_OBSERVER`, `RADAR` | Three parallel countings exist (RejectionStore, decision_packet.rejected_by, gate_rejections.csv) — reconcile before treating any single one as the canonical number to headline |
...
| PaperArena experiment metrics (IN_MEMORY_EXPERIMENT_STATE — resets on restart, no file-persistence call site found) | `src/paper/paper_metrics.py`, `paper_report.py` | `SCIENTIFIC_RESEARCH` | `PAPER_ARENA` | Independent research track — must be labeled distinctly from the main portfolio in any surface that shows both; do not present as a durable dataset |

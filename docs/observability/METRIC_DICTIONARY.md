...
| Telegram metric | Current source | Canonical mapping | Availability |
|---|---|---|---|
| Win rate, Sharpe, max/current drawdown, total trades (`/status`, `/kpis`) | `PhaseKPITracker`, fed exclusively by `paper_trading.recorder.get_recorder().get_trades()` (**paper trades**); `initial_capital=real_capital` is the base capital figure from `exec_engine.fetch_available_capital()` — its provenance is mode-dependent (see `/balance` row below), **not** necessarily "real" despite the variable name | `portfolio_state` (paper) — KPI population is unambiguously PAPER | **REMEDIATED (D-7, O-02B/O-02B-R1/R2)** — `/status` and `/kpis` now render an explicit `*PERFORMANCE PAPER*`/`*KPIs PAPER*` header next to these figures, and the base-capital figure shown alongside them carries the same declared provenance label as `/balance` (never a bare, unlabeled number). |
| `/balance` (spot/futures) | `exec_engine.fetch_available_capital()` -> `real_capital` | `portfolio_state.real_account_equity_usd`-adjacent **only when live/testnet** — under `PAPER_TRADING_ENABLED=true` (current stabilization default) this is the `WalletSync` **PAPER** balance despite the `real_capital` variable name (D-7, O-02B-R1) | CANONICAL_EXISTING; `command_center_bot.py` now labels the panel via `get_balance_provenance()` (PAPER/REAL_API/TESTNET_API/UNKNOWN) instead of a static "Real" assumption |

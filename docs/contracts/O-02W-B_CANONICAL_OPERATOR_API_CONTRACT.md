...
| `tp_price` / `sl_price` | YES. On **restore**, `_restore_positions()` uses the durably recorded original TP/SL (schema v4, REM-C R1) when the ledger record actually has them; only when that evidence is genuinely absent does it recompute from a fixed 4%/2% assumption — the two cases are NOT the same confidence tier and must not share a provenance tag (REM-C R1.3 correction: an earlier version of this contract conflated them). The snapshot MUST carry an explicit `tp_sl_source`: `"original"` (normal `_fill_market()` open, never restored), `"restored_original"` (restored position, but TP/SL are durably recorded evidence — `MexcPosition.restored_evidence_gaps` does NOT contain `"tp_sl_reconstructed_default"`), or `"restored_default"` (restored AND genuinely reconstructed — that gap IS present); `"restored_default"` must never be presented with the same confidence as an order-derived or durably-recovered value | |

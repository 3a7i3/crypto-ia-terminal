...
def _build_pinned_text() -> str:
    ...
    from visualization.renderers.base import bar_text, pct_to_color
    ...
    lines = [
        ...
        f"<code>Traités:  {p.n_traded}  |  Refusés: {p.n_refused}  |  Pass: {p.pass_rate_pct:.0f}%</code>",
    ]
    ...
    lines += [
        "",
        f"<code>{state_icon} {h.system_state}  {trade_icon}  N={h.n_trades}  WR={h.win_rate_pct:.0f}%</code>",
    ]
...
def _handle_command(text: str, chat_id: str):
    ...
    if cmd == "/start" or cmd == "/help":
        help_text = (
            ...
            + "\n".join(f"<code>{c}</code> — {desc}" for c, (desc, _) in COMMANDS.items())

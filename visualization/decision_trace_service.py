...
def label_for(blocker: str) -> str:
    """Résout un identifiant de bloqueur brut (ex: 'meta_strategy(score<66)')
    en libellé lisible."""
    key = blocker.split("(")[0]
...
@dataclass
class DecisionTrace:
    """Trace causale complète d'une évaluation de signal (entrée du RejectionStore)."""
...
class DecisionTraceService:
    ...
    def load_recent_entries(
        ...
        """Charge et concatène les entrées des `days` derniers jours
        (jusqu'à `end_day` inclus)."""
        end = end_day or date.today()
        out: list[dict] = []
        for i in range(days):
            out.extend(
                self.load_entries(
                    day=end - timedelta(days=i), symbol=symbol, cycle=cycle
                )
            )
    ...
    def build_trace(self, entry: dict[str, Any]) -> DecisionTrace:
        """Reconstruit la chaîne causale couche par couche pour une entrée
        du RejectionStore."""
        ...
        dp_detail = next(
            (b for b in all_blockers if "decision_packet" in b), "non évalué"
        )
        ...
        steps = [
            ...
            TraceStep(
                3,
                f"MetaStrategy ({entry.get('personality_name', 'unknown')})",
                meta_ok,
                meta_reason,
            ),
            TraceStep(
                4,
                "Conviction Engine",
                None if not conv_level else not conv_blocked,
                (
                    f"level={conv_level} score={entry.get('conviction_score')}"
                    if conv_level
                    else "non évalué"
                ),
            ),
            TraceStep(
                5,
                "Portfolio Brain",
                None if not pb_reason else not blocked_by("portfolio"),
                pb_reason or "non évalué",
            ),
            TraceStep(
                6,
                "Executive Override",
                None if not eo_level else not blocked_by("executive_override"),
                (
                    f"level={eo_level} reason={entry.get('override_reason')}"
                    if eo_level
                    else "non évalué"
                ),
            ),
            TraceStep(
                7,
                "MistakeMemory",
                None if not mm_reason else not blocked_by("mistake_memory"),
                mm_reason or "non évalué",
            ),
            TraceStep(
                8,
                "ThreatRadar",
                None if not radar_level else not blocked_by("threat_radar"),
                f"level={radar_level} threats={entry.get('radar_threat_count', 0)}",
            ),
            TraceStep(
                9,
                "DecisionArbitrator",
                None if not arb_decision else not blocked_by("arbitrator"),
                arb_decision or "non évalué",
            ),
            TraceStep(
                10,
                "DecisionPacket G8 guard",
                None if not dp_blocked else False,
                dp_detail,
            ),
        ]
    ...
    def statistics(self, entries: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        n = len(entries)
        by_layer_pct = (
            {k: round(100 * v / n, 1) for k, v in by_layer.items()} if n else {}
        )
    ...
    def timeline(
        self, days: int = 1, limit: int = 50, end_day: Optional[date] = None
    ) -> list[RejectionEvent]:
        """Les événements les plus récents du RejectionStore, du plus
    ...
    def rejections(
        self, days: int = 1, limit: int = 20, end_day: Optional[date] = None
    ) -> RejectionsSnapshot:
        """Vue combinée agrégats + événements récents — alimente le
        panneau Reject Analyzer."""
        end = end_day or date.today()

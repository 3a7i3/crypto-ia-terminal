...
class PipelineRenderer(BaseRenderer):
    ...
    def render(self, obj: object) -> bytes:
        ...
        fig.suptitle(
            f"SDOS PIPELINE — {p.ts.strftime('%Y-%m-%d %H:%M')} UTC  |  "
            f"Cycle {p.cycle}",
            color=SVL_TEXT,
            fontsize=12,
            fontweight="bold",
    ...
    def _draw_pipeline(self, ax, p: PipelineSnapshot):
        ...
        ax.text(
            0.5,
            0.18,
            f"Signals: {p.n_signals}  |  Traded: {p.n_traded}  |  "
            f"Refused: {p.n_refused}  |  Pass rate: {p.pass_rate_pct:.1f}%",
            ha="center",
            va="center",
            color=SVL_TEXT_DIM,
            fontsize=9,

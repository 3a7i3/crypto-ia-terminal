...
class EquityRenderer(BaseRenderer):
    ...
    def _draw_equity_curve(self, ax, p: PortfolioSnapshot):
        ...
        ax.set_ylabel(
            "Cumulative PnL ($)", color=SVL_TEXT_DIM, fontsize=8, fontfamily="monospace"
        )
    ...
    def _draw_no_data(self, ax, p: PortfolioSnapshot):
        ...
        ax.text(
            0.5,
            0.45,
            f"WR {p.win_rate_pct:.1f}%  |  PF {p.profit_factor:.2f}  |  "
            f"PnL ${p.total_pnl_usd:+.2f}",
            ha="center",
            va="center",
            color=pct_to_color(p.win_rate_pct),
            fontsize=11,
            fontfamily="monospace",
            transform=ax.transAxes,
        )

...
from visualization.renderers.base import (
    ...
    SVL_COLORS, pct_to_color, pct_to_icon, bar_text, SVL_DPI,
)
...
class SnapshotRenderer(BaseRenderer):
    ...
    def render(self, obj: object) -> bytes:
        ...
        fig.suptitle(
            f"{state_icon} SDOS SNAPSHOT — {b.health.ts.strftime('%Y-%m-%d %H:%M')} UTC",
            color=SVL_TEXT, fontsize=13, fontweight="bold",
    ...
    def _draw_mini_radar(self, ax, h: HealthSnapshot):
        axes_labels = ["Observer", "Dataset", "Knowledge", "Evidence", "Capital", "~Drift"]
        values = [h.observer_pct, h.dataset_pct, h.knowledge_pct,
                  h.evidence_pct, h.capital_pct, 100.0 - h.drift_pct]
        ...
        for lv in [25, 50, 75, 100]:
            ax.plot(angles_c, [lv] * (N + 1), color=SVL_GRID, linewidth=0.4, linestyle="--")
        for a in angles:
        ...
        ax.set_xticklabels(axes_labels, fontsize=7, fontfamily="monospace", color=SVL_TEXT)
        ax.set_ylim(0, 100)
        ax.set_yticks([])
        ax.spines["polar"].set_visible(False)
        ax.set_facecolor(SVL_BG)
        ax.set_title("System Health", color=SVL_TEXT, fontsize=9,
    ...
    def _draw_health_text(self, ax, h: HealthSnapshot):
        ...
        y = 0.92
        ax.text(0.02, 1.0, "Health Details", color=SVL_TEXT_DIM, fontsize=9,
        ...
        for label, pct in rows:
            ...
            ax.text(0.02, y, f"{label:<10} {bar}  {real_val:.0f}%  {icon}",
                    color=color, fontsize=8, fontfamily="monospace",
                    transform=ax.transAxes, va="top")
            y -= 0.11

        y -= 0.04
        state_color = SVL_COLORS["STRONG"] if h.system_state == "NORMAL" else SVL_COLORS["CONTRADICTED"]
        state_lbl = "[OK]" if h.system_state == "NORMAL" else "[!!]"
        ax.text(0.02, y, f"State: {state_lbl} {h.system_state}", color=state_color,
                fontsize=9, fontweight="bold", fontfamily="monospace",
                transform=ax.transAxes, va="top")
        y -= 0.11
        trade_color = pct_to_color(h.win_rate_pct)
        ax.text(0.02, y, f"N={h.n_trades}  WR={h.win_rate_pct:.0f}%  PF={h.profit_factor:.2f}",
                color=trade_color, fontsize=8, fontfamily="monospace",
                transform=ax.transAxes, va="top")
    ...
    def _draw_mini_pipeline(self, ax, p: PipelineSnapshot):
        ax.axis("off")
        ax.set_title("Decision Pipeline", color=SVL_TEXT, fontsize=9,
        ...
        for label, key in layers:
            ...
            rect = plt.Rectangle((x + gap / 2, y_bar - bar_h), bar_w - gap, bar_h,
                                  facecolor=color + "70", edgecolor=color,
                                  linewidth=1, transform=ax.transAxes)
            ax.add_patch(rect)

            ax.text(x + bar_w / 2, y_lbl, label,
                    ha="center", color=SVL_TEXT, fontsize=7, fontfamily="monospace",
                    transform=ax.transAxes)
            ax.text(x + bar_w / 2, y_pct - 0.06, f"{pass_pct:.0f}%",
        ...
        ax.text(0.5, 0.10,
                ...
                ha="center", color=SVL_TEXT_DIM, fontsize=8,
                fontfamily="monospace", transform=ax.transAxes)
    ...
    def _draw_portfolio_kpis(self, ax, p: PortfolioSnapshot):
        ax.axis("off")
        ax.set_title("Portfolio", color=SVL_TEXT, fontsize=9,
                     fontfamily="monospace", loc="left")

        kpis = [
            ("Trades",        f"{p.n_trades}",                   SVL_TEXT),
            ("Win Rate",      f"{p.win_rate_pct:.1f}%",          pct_to_color(p.win_rate_pct)),
            ("Profit Factor", f"{p.profit_factor:.2f}",          pct_to_color(min(p.profit_factor / 3 * 100, 100))),
            ("Max DD",        f"{p.max_drawdown_pct:.2f}%",      SVL_COLORS["DRIFT"]),
            ("Sharpe",        f"{p.sharpe:.2f}",                 SVL_TEXT),
            ("Total PnL",     f"${p.total_pnl_usd:+.2f}",       pct_to_color(60 if p.total_pnl_usd >= 0 else 30)),
        ...
        for i, (label, val, color) in enumerate(kpis):
            ...
            if col == 0 and i > 0:
                y -= 0.17
            ax.text(x, y, label, color=SVL_TEXT_DIM, fontsize=7.5,
                    fontfamily="monospace", transform=ax.transAxes)
            ax.text(x, y - 0.09, val, color=color, fontsize=10, fontweight="bold",
                    fontfamily="monospace", transform=ax.transAxes)

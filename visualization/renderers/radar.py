...
from visualization.renderers.base import (
    ...
    SVL_COLORS, pct_to_color, pct_to_icon, SVL_DPI, SVL_W, SVL_H,
)
...
class RadarRenderer(BaseRenderer):
    ...
    def render(self, obj: object) -> bytes:
        assert isinstance(obj, HealthSnapshot), f"RadarRenderer expects HealthSnapshot, got {type(obj)}"
        h = obj
    ...
    def _draw_radar(self, ax, angles, angles_closed, values_closed, values):
        ...
        for level in [25, 50, 75, 100]:
            ax.plot(
                ...
                [level] * (N + 1),
                color=SVL_GRID, linewidth=0.5, linestyle="--",
            )
            ax.text(
                0, level, f"{level}%",
                color=SVL_TEXT_DIM, fontsize=6, ha="center", va="center",
        ...
        ax.set_xticklabels(
            self.AXES, fontsize=9, fontfamily="monospace",
            color=SVL_TEXT,
        )
        ax.set_ylim(0, 100)
        ax.set_yticks([])
    ...
    def _draw_panel(self, ax, h: HealthSnapshot, values: list[float]):
        ...
        lines = []
        state_icon = "[OK]" if h.system_state == "NORMAL" else ("[!!]" if h.system_state == "HALTED" else "[ !]")
        lines.append(("", SVL_TEXT, 12, "bold"))
        lines.append((f"{state_icon} {h.system_state}  |  Trading: {'ON' if h.trading_enabled else 'OFF'}", SVL_TEXT, 10, "bold"))
        ...
        for label, val, orig in [
            ...
            lines.append((f"{label:<10} {bar} {display:>5.1f}%  {icon}", color, 9, "normal"))
        ...
        if h.top_root_cause:
            ...
            lines.append((f"  {h.top_root_cause}  {h.top_root_cause_pct:.0f}%", SVL_COLORS["DRIFT"], 9, "bold"))

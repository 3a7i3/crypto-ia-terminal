...
from visualization.api.models import PipelineSnapshot, RegimeSnapshot
from visualization.renderers.base import (  # noqa: F401
    SVL_BG,
    SVL_COLORS,
    SVL_DPI,
    SVL_GRID,
...
class TimelineRenderer(BaseRenderer):
    ...
    def _draw_bars(self, ax, distribution: dict[str, int]):
        ...
        # Legend
        handles = [
            mpatches.Patch(color=_regime_color(r), label=r) for r, _ in items[:5]
        ]
        ax.legend(
            handles=handles,
            loc="lower right",
